...
<script id="v0781StabilityHubScript">
(()=>{
  if(globalThis.__v0781ObserverHub)return;
  const callbacks=new Set();
  let queued=false,lastRecords=[],flushes=0,callbackRuns=0;
  const nativeObserver=new MutationObserver((records)=>{
    ...
    setTimeout(()=>{
      ...
      for(const cb of Array.from(callbacks)){
        try{callbackRuns++;cb(rs,nativeObserver)}catch(e){try{console.warn('[v0.7.87 observer callback]',e)}catch(_e){}}
      }
    },90);
  });
  const root=document.getElementById('app')||document.body||document.documentElement;
  try{nativeObserver.observe(root,{childList:true,subtree:true,characterData:true})}catch(_e){try{nativeObserver.observe(document.documentElement,{childList:true,subtree:true,characterData:true})}catch(__e){}}
  class SharedObserver{
    constructor(cb){this.cb=typeof cb==='function'?cb:null;this.active=false}
    observe(){if(this.cb&&!this.active){this.active=true;callbacks.add(this.cb)}}
    disconnect(){if(this.cb){callbacks.delete(this.cb);this.active=false}}
    takeRecords(){return []}
  }
  ...
  console.info('Palworld OCG v0.7.87 stability hub: shared MutationObserver active');
})();
</script>
...
(()=>{
  ...
  function apply(){
    ...
    try{
      ...
      if(title)title.textContent='v0.7.87';

      /* 起動画面のバージョン表示もWorker実装と同期 */
      document.querySelectorAll('.badge.official').forEach(b=>{
        if(/^v?0\.7\.\d+/.test((b.textContent||'').trim())) b.textContent='v0.7.87';
      });
  ...
  try{
    new globalThis.v0781SharedObserver(schedule).observe(document.documentElement,{childList:true,subtree:true});
  }catch(_e){}
...
/* v0.7.87 — fixed interrupt sheet: no vertical jump during re-render/image load. */
.modal.v0787QuickModal{overflow-anchor:none!important;overscroll-behavior:none!important}
.modal.v0745CombatModal.v0787QuickModal>.modalCard{height:min(228px,38dvh)!important;min-height:min(188px,34dvh)!important;max-height:none!important;overflow:hidden!important;display:flex!important;flex-direction:column!important;contain:layout paint style!important;overflow-anchor:none!important}
.modal.v0745CombatModal.v0787QuickModal .v0745CombatHead{flex:0 0 auto!important;margin-bottom:3px!important}
.v0787QuickList{display:flex!important;flex:1 1 auto!important;min-height:0!important;gap:6px!important;overflow-x:auto!important;overflow-y:hidden!important;align-items:stretch!important;scroll-snap-type:x proximity!important;overflow-anchor:none!important;overscroll-behavior-x:contain!important;padding:1px 1px 3px!important}
.modal.v0745CombatModal.v0787QuickModal .v0745QuickRow{flex:0 0 min(275px,68vw)!important;width:min(275px,68vw)!important;min-width:min(275px,68vw)!important;height:116px!important;min-height:116px!important;max-height:116px!important;grid-template-columns:72px minmax(0,1fr)!important;gap:6px!important;border:1px solid #29463a!important;border-radius:9px!important;padding:4px!important;scroll-snap-align:start!important;box-sizing:border-box!important;overflow:hidden!important;overflow-anchor:none!important}
.modal.v0745CombatModal.v0787QuickModal .v0745QuickRow .card{height:102px!important;width:66px!important;min-width:66px!important;max-width:66px!important;margin:0!important}
.modal.v0745CombatModal.v0787QuickModal .v0745QuickBtns{gap:4px!important}
.modal.v0745CombatModal.v0787QuickModal .v0745QuickBtns button{min-height:32px!important;padding:5px 3px!important;line-height:1.12!important}
.modal.v0745CombatModal.v0787QuickModal .v0745CombatActions{flex:0 0 auto!important;position:static!important;margin-top:3px!important;padding-top:3px!important}
@media(max-height:520px) and (orientation:landscape){.modal.v0745CombatModal.v0787QuickModal>.modalCard{height:178px!important;min-height:178px!important;max-height:178px!important;width:min(460px,74vw)!important}.modal.v0745CombatModal.v0787QuickModal .v0745QuickRow{height:92px!important;min-height:92px!important;max-height:92px!important;grid-template-columns:58px minmax(0,1fr)!important;flex-basis:min(250px,66vw)!important;width:min(250px,66vw)!important;min-width:min(250px,66vw)!important}.modal.v0745CombatModal.v0787QuickModal .v0745QuickRow .card{height:82px!important;width:52px!important;min-width:52px!important;max-width:52px!important}.modal.v0745CombatModal.v0787QuickModal .v0745QuickBtns button{min-height:27px!important;padding:3px 2px!important;font-size:7px!important}}
...
(()=>{
  ...
  if(typeof quickModalHTML==='function'){
    quickModalHTML=function(){
      ...
      return '<div class="modal v0745CombatModal v0787QuickModal"><div class="modalCard">'+
        '<div class="v0745CombatHead"><div><h2>割り込みを使いますか？</h2><div class="v0745CombatSub">相手ターンのバトル中。使うカードとコストを選択してください。</div></div></div>'+
        '<div class="v0787QuickList">'+rows+'</div><div class="v0745CombatActions"><button class="v0745Skip" onclick="skipInterrupt()">使わない</button></div>'+
        '</div></div>';
    };
  }
  /* v0.7.87: normal attack arrows are owned only by the integrated battle core. */
  ...
  if(typeof chooseInterrupt==='function'){
    ...
    function v0787HideQuickSheet(){
      try{document.querySelectorAll('.modal.v0745CombatModal').forEach(m=>{if(/割り込みを使いますか/.test(m.textContent||'')){m.style.visibility='hidden';m.style.pointerEvents='none'}})}catch(_e){}
    }
    chooseInterrupt=function(uid,mode){
      const before=pendingQuick;
      if(!before)return;
      let name='',card=null;try{card=before?.ints?.find(x=>x.uid===uid)||null;name=card?.name||''}catch(_e){}
      if(!card){toast('割り込みカードを確認できません');return}
      try{
        if(mode==='soul'&&standingSouls(G.p)<1){toast('ソウルが足りません');return}
        if(mode==='discard'&&G.p.hand.filter(x=>x.uid!==uid).length<1){toast('追加で捨てる手札がありません');return}
      }catch(_e){}
      v0787HideQuickSheet();
      const hadCard=()=>{try{return G.p.hand.some(x=>x.uid===uid)}catch(_e){return true}};
      let r;
      try{r=baseChooseInterrupt(uid,mode)}catch(e){try{render()}catch(_e){};toast('割り込み処理でエラー');throw e}
      let success=false;
      try{
        if(mode==='soul')success=!hadCard()||pendingQuick!==before;
        else success=pendingQuick!==before&&!!pendingChoice;
      }catch(_e){}
      if(success){
        requestAnimationFrame(()=>{try{document.querySelectorAll('.modal.v0745CombatModal').forEach(m=>{if(/割り込みを使いますか/.test(m.textContent||''))m.remove()})}catch(_e){}});
        if(mode==='discard')toast('<b>INTERRUPT</b> 追加で捨てる手札を選択');
        else toast('<b>INTERRUPT</b> '+safeText(name));
      }else{
        setTimeout(()=>{try{if(pendingQuick===before)render()}catch(_e){}},0);
        toast('割り込みを使用できません');
      }
      return r;
    };
  ...
  function applyCombatUX(){
    ...
    try{
      ...
      try{if(!pendingQuick&&pendingChoice)document.querySelectorAll('.modal.v0745CombatModal').forEach(m=>{if(/割り込みを使いますか/.test(m.textContent||''))m.remove()})}catch(_e){}
      const play=playRoot();
      if(!play)return;
...
/* v0.7.48: a long-press detail is a temporary preview, not a blocking modal.
  ...
  try{new globalThis.v0781SharedObserver(scheduleCombatUX).observe(document.documentElement,{childList:true,subtree:true})}catch(_e){}
...
(()=>{
  ...
  try{new globalThis.v0781SharedObserver(schedule).observe(document.documentElement,{childList:true,subtree:true})}catch(_e){}
...
(()=>{
  ...
  function sync(){queued=false;markChoiceSheets()}
  function schedule(){if(queued)return;queued=true;requestAnimationFrame(sync)}
  try{new globalThis.v0781SharedObserver(schedule).observe(document.documentElement,{childList:true,subtree:true,characterData:true})}catch(_e){}
  document.addEventListener('pointerup',schedule,{passive:true});
  addEventListener('pageshow',sync,{passive:true});
  addEventListener('resize',schedule,{passive:true});
  sync();setTimeout(sync,250);setTimeout(sync,900);
  console.info('Palworld OCG v0.7.87 compact effect-choice UI applied (legacy DAMAGE CHECK styling disabled)');
})();
</script>
...
(()=>{
  ...
  function sync(){
    queued=false;
    try{
      const title=document.querySelector('.v04Title');if(title)title.textContent='v0.7.87';
      document.querySelectorAll('.badge.official').forEach(function(b){if(/^v?0\.7\.\d+/.test((b.textContent||'').trim()))b.textContent='v0.7.87'});
      syncButton();
    }catch(_e){}
  }
  function schedule(){if(queued)return;queued=true;requestAnimationFrame(sync)}
...
(()=>{
  ...
  function isInteresting(msg){
    if(!msg)return false;
    if(/ダメージチェック|Damage Check|Lucky Pal|ライフ-\d+/i.test(msg))return false;
  ...
  function classify(msg){
    if(/ダメージチェック/.test(msg))return 'ACTION';
...
(()=>{
  ...
  // v0.7.87: remove old successful test-play entries that were mistakenly stored as errors.
  ...
  try{new globalThis.v0781SharedObserver(()=>requestAnimationFrame(syncButton)).observe(document.documentElement,{childList:true,subtree:true})}catch(_e){}
...
(()=>{
  ...
  const VERSION='0.7.87', STORE='palworld_suite_v0763';
  ...
  try{new globalThis.v0781SharedObserver(sync).observe(document.documentElement,{childList:true,subtree:true})}catch(_e){}
...
(()=>{
  ...
  try{new globalThis.v0781SharedObserver(()=>{if(scanQueued)return;scanQueued=true;requestAnimationFrame(()=>{scanQueued=false;wrapTierFunctions();wrapSetup('setupCpuVsCpu');patchValidate();patchPersist();syncPanel()})}).observe(document.documentElement,{childList:true,subtree:true})}catch(_e){}
  ...
  console.info('Palworld OCG v0.7.87 Tier legal-deck guard applied with BP01 sync defer');
})();
</script>
...
/* v0.7.87 — stable serialized battle visuals. Attack cue DOM is reused and DAMAGE CHECK is sequenced. */
...
@keyframes v0781Ring{from{filter:brightness(.88)}to{filter:brightness(1.25)}}
#v0781AttackLabel{position:fixed;left:50%;top:9%;transform:translateX(-50%);max-width:82vw;padding:8px 14px;border-radius:999px;border:2px solid #e8c852;background:#07130ff4;color:#fff;font:1000 14px/1.2 sans-serif;text-align:center;box-shadow:0 7px 22px #000c;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
.v0781DamageStage:not(.loaded)::before{content:'CARD IMAGE';font:900 11px/1 sans-serif;color:#cfd8d3;opacity:.55;letter-spacing:.08em}
...
(()=>{
  ...
  globalThis.v0781CpuTurnLeadDelay=function(){if(stress())return 10;return ({slow:1250,normal:750,fast:350,instant:20})[mode()]};
  function trace(code,data){
    try{const key='palworld_v0781_battle_trace',a=JSON.parse(localStorage.getItem(key)||'[]');a.unshift({at:new Date().toISOString(),code,data:data||null});localStorage.setItem(key,JSON.stringify(a.slice(0,100)))}catch(_e){}
    ...
    try{console.info('[v0781]',code,data||'')}catch(_e){}
  }
  globalThis.palBattleTraceReport=()=>{try{return localStorage.getItem('palworld_v0781_battle_trace')||'[]'}catch(_e){return'[]'}};
  function statusCheck(){const st=globalThis.__v0781SourcePatchStatus||{},missing=Object.keys(st).filter(k=>!st[k]);if(missing.length){let e=document.getElementById('v0781CoreError');if(!e){e=document.createElement('div');e.id='v0781CoreError';document.body.appendChild(e)}e.textContent='ERR-CORE-0001 戦闘コア直挿し失敗: '+missing.join(', ');trace('ERR-CORE-0001',{missing,status:st})}else trace('CORE-PATCH-OK',st)}
  ...
  function artSources(c){
    ...
    try{if(typeof EMBEDDED_CARD_IMAGES!=='undefined'&&c?.no)push('EMBEDDED',EMBEDDED_CARD_IMAGES[c.no])}catch(_e){}
    try{if(typeof v050ImageUrls!=='undefined'&&c?.no)push('BP_CACHE',v050ImageUrls[c.no])}catch(_e){}
    try{push('IMAGE_URL',c?.imageUrl);push('FALLBACK_URL',c?.imageFallbackUrl)}catch(_e){}
    try{if(typeof imageCandidates==='function'&&c?.no)for(const u of imageCandidates(c.no)||[])push('CANDIDATE',u)}catch(_e){}
    return out;
  }
  function fallbackCard(stage,c,reason){stage.style.backgroundImage='';stage.innerHTML='';try{if(typeof cardHTML==='function'){const wrap=document.createElement('div');wrap.innerHTML=cardHTML(c,{});const card=wrap.firstElementChild;if(card){card.removeAttribute('onclick');card.querySelectorAll('button,.infoBtn,.actBtn').forEach(n=>n.remove());stage.appendChild(card);stage.classList.add('loaded');trace('DMG-ART-CARD-001',{card:c?.no||c?.name,reason});return}}}catch(_e){}const d=document.createElement('div');d.style.cssText='padding:35px 10px;font-weight:900';d.textContent=c?.name||c?.no||'公開カード';stage.appendChild(d)}
  function setStageSource(stage,c,items,index=0){
    if(!stage?.isConnected)return;
    if(index>=items.length){
      if(c?.no&&String(c.no).startsWith('BP01-')&&typeof v050DbGet==='function'){
        Promise.resolve(v050DbGet(c.no)).then(b=>{if(!stage?.isConnected)return;if(!b){trace('ERR-DMG-ART-079',{card:c?.no||c?.name,reason:'no usable image source or IndexedDB blob'});fallbackCard(stage,c,'no image source');return}try{const u=URL.createObjectURL(b);setStageSource(stage,c,[{route:'IDB',src:u}],0)}catch(e){trace('ERR-DMG-ART-079',{card:c?.no||c?.name,reason:String(e)});fallbackCard(stage,c,'IDB object URL failed')}});return
      }
      trace('ERR-DMG-ART-079',{card:c?.no||c?.name,reason:'all image routes failed',sources:items.map(x=>x.route)});fallbackCard(stage,c,'all image routes failed');return;
    }
    const it=items[index];stage.style.backgroundImage='url('+JSON.stringify(it.src)+')';stage.innerHTML='';
    const probe=new Image();let settled=false;
    const ok=()=>{if(settled||!stage?.isConnected)return;settled=true;stage.classList.add('loaded');trace('DMG-ART-SUCCESS-079',{card:c?.no||c?.name,route:it.route,width:probe.naturalWidth,height:probe.naturalHeight})};
    probe.onload=ok;probe.onerror=()=>{if(settled)return;settled=true;stage.style.backgroundImage='';trace('DMG-ART-FAIL-ROUTE-079',{card:c?.no||c?.name,route:it.route});setStageSource(stage,c,items,index+1)};probe.src=it.src;
  ...
  function fallbackDamageFromLogs(){
    if(Date.now()-lastQueueAt<450)return;
    try{
      const logs=Array.isArray(G?.logs)?G.logs:[];let ri=-1,count=1,def=null;
      for(let i=0;i<Math.min(logs.length,12);i++){const t=String(logs[i]||'');let m=t.match(/^(YOU|CPU)\s*:\s*Damage Check\s+(\d+)枚/i);if(m){ri=i;count=Math.max(1,Number(m[2])||1);def=m[1]==='YOU'?G.p:G.a;break}m=t.match(/^(YOU|CPU)\s*:\s*Lucky Pal!/i);if(m){ri=i;def=m[1]==='YOU'?G.p:G.a;count=1;break}}
      if(ri<0||!def)return;
      const names=[];for(let i=ri+1;i<Math.min(logs.length,ri+10)&&names.length<count;i++){const t=String(logs[i]||''),m=t.match(/^(YOU|CPU)\s*:\s*(.*?)\s*→\s*墓地\s*\(Damage Check\)/i);if(m&&((def===G.p&&m[1]==='YOU')||(def===G.a&&m[1]==='CPU')))names.push(m[2])}
  ...
  function version(){try{const t=document.querySelector('.v04Title');if(t&&!G?.cpuVsCpu)t.textContent='v0.7.87';document.querySelectorAll('.badge.official').forEach(b=>{if(/^v?0\.7\.\d+/.test(flat(b.textContent)))b.textContent='v0.7.87'})}catch(_e){}}
  ...
  console.info('Palworld OCG v0.7.87 integrated battle visuals loaded: stable single-draw attack cue + serialized DAMAGE CHECK');
})();
</script>

<style id="v0781TestPlayStyle">
#v0781TestOverlay{position:fixed;inset:0;z-index:2147483550;background:#000b;display:flex;align-items:center;justify-content:center;padding:10px}
#v0781TestOverlay[hidden]{display:none!important}.v0781TestBox{width:min(760px,94vw);max-height:94vh;overflow:hidden;display:grid;grid-template-rows:auto auto minmax(0,1fr) auto;gap:8px;background:#0b2119;border:2px solid #7dd8ad;border-radius:16px;padding:12px;color:#eefaf4;box-shadow:0 18px 60px #000d}.v0781TestHead{display:flex;align-items:center;justify-content:space-between;gap:8px}.v0781TestHead h2{margin:0;font-size:18px}.v0781TestMeta{font-size:9px;color:#b9d8c8}.v0781TestTools{display:flex;gap:8px;align-items:center;flex-wrap:wrap;background:#102d22;border:1px solid #416d59;border-radius:10px;padding:7px;font-size:10px}.v0781TestTools select{font-size:10px;padding:5px}.v0781TestList{min-height:0;overflow:auto;border:1px solid #355f4d;border-radius:10px;background:#07150f;padding:5px}.v0781TestRow{display:grid;grid-template-columns:76px minmax(0,1fr) 74px;gap:7px;align-items:center;padding:5px;border-bottom:1px solid #234334}.v0781TestRow:last-child{border-bottom:0}.v0781TestNo{font-size:8px;color:#a9c8b8}.v0781TestName{font-size:10px;font-weight:850}.v0781TestCopies{font-size:7px;color:#8fb2a1}.v0781TestRow select{width:70px;padding:4px;font-size:9px}.v0781TestFoot{display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap}.v0781TestCount{font-size:10px;font-weight:900;color:#ffe695}.v0781TestActions{display:flex;gap:6px;flex-wrap:wrap}.v0781TestActions button,.v0781TestEntry{font-weight:900}.v0781TestStart{background:#176448!important;border-color:#88e4b7!important}.v0781TestReady{width:min(900px,94vw)!important}.v0781TestReady .hand{display:flex;gap:5px;justify-content:center;overflow:auto;max-height:48vh}.v0781TestReady .card{flex:0 0 auto}
@media(max-height:560px) and (orientation:landscape){.v0781TestBox{max-height:96vh;padding:7px;gap:5px}.v0781TestHead h2{font-size:14px}.v0781TestTools{padding:4px;font-size:8px}.v0781TestRow{grid-template-columns:62px minmax(0,1fr) 62px;padding:3px}.v0781TestName{font-size:8px}.v0781TestRow select{width:60px;padding:2px;font-size:8px}.v0781TestActions button{font-size:8px;padding:5px 7px}}
</style>
<script id="v0781TestPlayScript">
(()=>{
  if(globalThis.__v0781TestPlayApplied)return;globalThis.__v0781TestPlayApplied=true;
  const KEY='palworld_test_play_v0781';
  let launching=false,memoryConfig={};
  function h(v){try{return typeof esc==='function'?esc(String(v??'')):String(v??'').replace(/[&<>\"]/g,function(ch){return {'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[ch]||ch})}catch(_e){return String(v??'')}}
  function load(){try{const x=JSON.parse(localStorage.getItem(KEY)||'{}');if(x&&typeof x==='object'&&Object.keys(x).length){memoryConfig=x;return x}}catch(_e){}return memoryConfig&&typeof memoryConfig==='object'?memoryConfig:{}}
  function save(x){memoryConfig=x&&typeof x==='object'?x:{};try{localStorage.setItem(KEY,JSON.stringify(memoryConfig))}catch(_e){}}
  function currentDeckKey(){return String(document.getElementById('pdeck')?.value||'TD01')}
  function deckInfo(key){try{if(typeof v047LoadSavedDecks==='function')v047LoadSavedDecks()}catch(_e){};try{return DECKS?.[key]||null}catch(_e){return null}}
  ...
  globalThis.v0781TestCountChanged=function(sel){
    ...
    if(total>10){sel.value=sel.getAttribute('data-prev')||'0';alert('テスト初手は最大10枚です');return v0781TestCountChanged(sel)}
    sel.setAttribute('data-prev',sel.value);const x=document.getElementById('v0781TestSelectedCount');if(x)x.textContent=String(total)+'枚選択';
  };
  function options(n,selected){let out='';for(let i=0;i<=n;i++)out+='<option value="'+i+'" '+(i===selected?'selected':'')+'>'+i+'枚</option>';return out}
  globalThis.v0781OpenTestPlay=function(){
    const key=currentDeckKey(),d=deckInfo(key);if(!d||!Array.isArray(d.main)||!d.main.length){alert('選択中のデッキを読み込めません。BP01入りデッキの場合は同期完了後にもう一度開いてください。');return}
    document.getElementById('v0781TestOverlay')?.remove();
    const prev=load(),same=prev.deckKey===key,counts=same&&prev.hand&&typeof prev.hand==='object'?prev.hand:{};
    let rows='',total=0;
    for(const row of d.main){const no=String(row[0]||''),name=String(row[1]||no),copies=Math.max(0,Number(row[2])||0),sel=Math.min(copies,Math.max(0,Number(counts[no])||0));total+=sel;rows+='<div class="v0781TestRow"><div class="v0781TestNo">'+h(no)+'</div><div><div class="v0781TestName">'+h(name)+'</div><div class="v0781TestCopies">デッキ内 '+copies+'枚</div></div><select data-no="'+h(no)+'" data-prev="'+sel+'" onchange="v0781TestCountChanged(this)">'+options(copies,sel)+'</select></div>'}
    const ov=document.createElement('div');ov.id='v0781TestOverlay';
    const soul=same&&Number.isFinite(Number(prev.souls))?Math.max(0,Math.min(10,Number(prev.souls))):10;
    ov.innerHTML='<div class="v0781TestBox"><div class="v0781TestHead"><div><h2>🧪 テストプレイ設定</h2><div class="v0781TestMeta">通常CPU対戦には影響しません / YOU側だけ初期状態を変更</div></div><button onclick="v0781CloseTestPlay()">閉じる</button></div><div class="v0781TestTools"><input id="v0781TestDeckKey" type="hidden" value="'+h(key)+'"><b>デッキ: '+h(d.name||key)+'</b><label>開始時ソウル <select id="v0781TestSouls">'+Array.from({length:11},function(_,i){return '<option value="'+i+'" '+(i===soul?'selected':'')+'>'+i+(i===10?' (MAX)':'')+'</option>'}).join('')+'</select></label><label><input id="v0781TestFill" type="checkbox" '+((same?prev.fillToFive!==false:true)?'checked':'')+'> 5枚未満ならランダムで5枚まで補充</label></div><div class="v0781TestList">'+rows+'</div><div class="v0781TestFoot"><div id="v0781TestSelectedCount" class="v0781TestCount">'+total+'枚選択</div><div class="v0781TestActions"><button onclick="v0781SaveTestPlay()">設定保存</button><button class="v0781TestStart" onclick="v0781StartConfiguredTest()">この設定でテスト開始</button></div></div></div>';
    document.body.appendChild(ov)
  };
  globalThis.v0781CloseTestPlay=function(){document.getElementById('v0781TestOverlay')?.remove()};
  globalThis.v0781SaveTestPlay=function(){const c=selectedConfigFromDom();if(c.total>10)return alert('初手は最大10枚です');save(c);const x=document.getElementById('v0781TestSelectedCount');if(x)x.textContent=c.total+'枚選択 / 保存済み'};
  globalThis.v0781StartConfiguredTest=function(){
    const c=selectedConfigFromDom();if(c.total>10)return alert('初手は最大10枚です');save(c);
    const pkey=currentDeckKey();if(c.deckKey!==pkey)return alert('YOUデッキが変更されています。テスト設定を開き直してください。');
    const p=document.getElementById('pdeck'),a=document.getElementById('adeck'),f=document.getElementById('first'),d=document.getElementById('diff');
    if(!p||!a||!f||!d)return alert('開始画面の設定を取得できません');
    launching=true;v0781CloseTestPlay();setupGame(p.value,a.value,f.value,d.value)
  };
  function applyConfig(){
    if(!launching||!G?.p)return false;launching=false;
    const c=load();if(!c||c.deckKey!==G.p.key)return false;
    const pl=G.p,pool=[...(pl.hand||[]),...(pl.deck||[])],fixed=[];let missing=[];
    for(const [no,n0] of Object.entries(c.hand||{})){
      const n=Math.max(0,Number(n0)||0);for(let k=0;k<n;k++){const i=pool.findIndex(function(x){return x?.no===no});if(i<0){missing.push(no);break}fixed.push(pool.splice(i,1)[0])}
    }
    if(c.fillToFive!==false){while(fixed.length<5&&pool.length)fixed.push(pool.shift())}
    pl.hand=fixed;pl.deck=typeof shuffle==='function'?shuffle(pool):pool;
    pl.souls=[];pl.soulDeck=10;const soul=Math.max(0,Math.min(10,Number(c.souls)||0));if(typeof addSouls==='function')addSouls(pl,soul);pl.souls.forEach(function(x){x.rest=false});
    pl.mulligan=false;
    try{log('[TEST] YOU 初期設定: ソウル'+soul+' / 手札'+pl.hand.length+'枚'+(missing.length?' / 不足:'+Array.from(new Set(missing)).join(',') : ''))}catch(_e){}
    try{const meta={souls:soul,hand:pl.hand.map(function(x){return x.no}),missing:missing};if(missing.length&&typeof palDiagRecord==='function')palDiagRecord('ERR-TEST-PLAY-0782','テスト初手の指定カードが見つかりません',meta,null,false);else console.info('[TEST-PLAY-0782] 初期設定適用成功',meta)}catch(_e){}
    return true
  }
  ...
  enhanceStart();
  console.info('Palworld OCG v0.7.87 test-play mode loaded');
})();
</script>


<style id="v0783MulliganDetailStyle">
.v0783MullRoot .hand .card[data-uid]{cursor:pointer!important;transition:transform .12s ease,box-shadow .12s ease!important}
.v0783MullRoot .hand .card[data-uid]:active{transform:scale(.96)!important}
.v0783MullRoot .hand .card[data-uid]:focus-visible{outline:2px solid #8cf0bd!important;outline-offset:2px!important}
.v0783MullRoot .hand .infoBtn{display:none!important}
.v0783MullHint{margin:6px 0 2px;color:#bfe7d0;font-size:10px;font-weight:800;letter-spacing:.02em}
.v0783MullDetailOverlay{position:fixed;inset:0;z-index:2147483646;background:rgba(0,8,6,.94);backdrop-filter:blur(5px);display:flex;align-items:center;justify-content:center;padding:10px}
.v0783MullDetailPanel{position:relative;width:min(1050px,96vw);height:min(650px,94dvh);border:2px solid #79d3a4;border-radius:18px;background:linear-gradient(155deg,#102a22,#07150f 72%);box-shadow:0 18px 70px #000;padding:12px;overflow:hidden}
.v0783MullDetailClose{position:absolute;right:8px;top:8px;z-index:3;width:42px;height:42px;border-radius:50%;font-size:26px;line-height:1;padding:0!important;background:#173d30!important;border:1px solid #80d7ae!important;color:#fff!important}
.v0783MullDetailBody{height:100%;display:grid;grid-template-columns:minmax(220px,42%) minmax(0,1fr);gap:14px;padding-right:48px;min-height:0}
.v0783MullDetailArt{min-height:0;border:1px solid #48715e;border-radius:13px;background:#06110d;display:flex;align-items:center;justify-content:center;overflow:hidden;padding:6px}
.v0783MullDetailArt img{width:100%;height:100%;object-fit:contain;display:block}
.v0783MullFallback{font-size:16px;font-weight:900;padding:18px;text-align:center;color:#d8efe3}
.v0783MullDetailInfo{min-height:0;overflow:auto;padding:4px 3px 8px}
.v0783MullDetailNo{font-size:11px;color:#9fc5b3;font-weight:800;margin-bottom:5px}
.v0783MullDetailName{font-size:clamp(20px,2.6vw,34px);font-weight:1000;line-height:1.15;margin:0 0 8px;color:#f4fff9}
.v0783MullDetailStats{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px}
.v0783MullDetailStats span{border:1px solid #567d6a;background:#0d2119;border-radius:8px;padding:5px 8px;font-size:11px;font-weight:900}
.v0783MullDetailAbility{white-space:pre-wrap;border:1px solid #3f6755;background:#091913;border-radius:11px;padding:10px;font-size:13px;line-height:1.55;color:#e4f2eb}
.v0783MullDetailTip{font-size:9px;color:#9db9aa;margin-top:8px}
@media(max-height:560px) and (orientation:landscape){
.v0783MullHint{font-size:8px;margin:3px 0 0}.v0783MullDetailOverlay{padding:4px}.v0783MullDetailPanel{width:98vw;height:96dvh;padding:7px;border-radius:12px}.v0783MullDetailClose{right:4px;top:4px;width:35px;height:35px;font-size:22px}.v0783MullDetailBody{grid-template-columns:minmax(175px,38%) minmax(0,1fr);gap:7px;padding-right:38px}.v0783MullDetailName{font-size:17px;margin-bottom:5px}.v0783MullDetailStats{margin-bottom:6px}.v0783MullDetailStats span{font-size:9px;padding:3px 5px}.v0783MullDetailAbility{font-size:11px;line-height:1.4;padding:7px}
}
</style>
<script id="v0783MulliganCardDetailScript">
(()=>{
  if(globalThis.__v0783MulliganCardDetailApplied)return;
  globalThis.__v0783MulliganCardDetailApplied=true;
  const safe=function(v){return String(v==null?'':v).replace(/[&<>\"']/g,function(ch){return {'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[ch]})};
  function getCard(uid){try{return G&&G.p&&Array.isArray(G.p.hand)?G.p.hand.find(function(c){return String(c&&c.uid)===String(uid)})||null:null}catch(_e){return null}}
  function statVal(fn,c,fallback){try{return typeof fn==='function'?fn(c):fallback}catch(_e){return fallback}}
  function closeDetail(){var old=document.getElementById('v0783MullDetailOverlay');if(old)old.remove()}
  function openDetail(uid){
    var c=getCard(uid);if(!c)return;
    closeDetail();
    var p=statVal(typeof powerOf==='function'?powerOf:null,c,c.power||0);
    var st=statVal(typeof strikeOf==='function'?strikeOf:null,c,c.strike||0);
    var dur=statVal(typeof durabilityOf==='function'?durabilityOf:null,c,c.power||0);
    var art='';try{if(typeof imageTag==='function')art=imageTag(c,'cardArt')||''}catch(_e){}
    var stats=['COST '+(Number(c.cost)||0)];
    if(c.kind==='Pal'){stats.push('POWER '+p);stats.push('STRIKE '+st)}else if(c.kind==='Structure'){stats.push('DUR '+dur)}
    if(c.lucky)stats.push('LUCKY');if(c.quick)stats.push('QUICK');
    var artHtml=art||('<div class="v0783MullFallback">'+safe(c.name)+'</div>');
    var ov=document.createElement('div');ov.id='v0783MullDetailOverlay';ov.className='v0783MullDetailOverlay';
    ov.innerHTML='<div class="v0783MullDetailPanel" role="dialog" aria-modal="true" aria-label="カード詳細"><button type="button" class="v0783MullDetailClose" aria-label="閉じる">×</button><div class="v0783MullDetailBody"><div class="v0783MullDetailArt">'+artHtml+'</div><div class="v0783MullDetailInfo"><div class="v0783MullDetailNo">'+safe(c.no)+' / '+safe(c.kind||'Card')+'</div><div class="v0783MullDetailName">'+safe(c.name)+'</div><div class="v0783MullDetailStats">'+stats.map(function(x){return '<span>'+safe(x)+'</span>'}).join('')+'</div><div class="v0783MullDetailAbility">'+safe(c.ability||'能力なし')+'</div><div class="v0783MullDetailTip">× または背景をタップするとマリガン画面へ戻ります。</div></div></div></div>';
    document.body.appendChild(ov);
    var x=ov.querySelector('.v0783MullDetailClose');if(x)x.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();closeDetail()});
    ov.addEventListener('click',function(e){if(e.target===ov)closeDetail()});
  }
  globalThis.v0783OpenMulliganDetail=openDetail;
  globalThis.v0783CloseMulliganDetail=closeDetail;
  function mullRoot(){
    var direct=document.querySelector('.v060Mull');if(direct)return direct;
    var xs=document.querySelectorAll('.modalCard');for(var i=0;i<xs.length;i++){var h=xs[i].querySelector('h2');if(h&&/初手/.test(h.textContent||'')&&xs[i].querySelector('.hand .card[data-uid]'))return xs[i]}
    return null;
  }
  function bind(){
    var root=mullRoot();if(!root)return;
    root.classList.add('v0783MullRoot');
    var good=root.querySelector('.good');if(good&&!root.querySelector('.v0783MullHint')){var hint=document.createElement('div');hint.className='v0783MullHint';hint.textContent='カードをタップすると詳細を確認できます。';good.insertAdjacentElement('afterend',hint)}
    root.querySelectorAll('.hand .card[data-uid]').forEach(function(card){
      var uid=card.getAttribute('data-uid');if(!uid||card.dataset.v0783DetailBound==='1')return;
      card.dataset.v0783DetailBound='1';card.setAttribute('role','button');card.setAttribute('tabindex','0');card.setAttribute('aria-label','カード詳細を表示');
      var ib=card.querySelector('.infoBtn');if(ib){ib.removeAttribute('onclick');ib.setAttribute('aria-hidden','true')}
      card.addEventListener('click',function(e){if(e.target&&e.target.closest&&e.target.closest('button'))return;e.preventDefault();e.stopPropagation();openDetail(uid)});
      card.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();openDetail(uid)}});
    });
  }
  try{if(typeof renderMulligan==='function'){var base=renderMulligan;renderMulligan=function(){var r=base.apply(this,arguments);requestAnimationFrame(bind);setTimeout(bind,80);return r}}}catch(e){console.warn('[v0.7.87 mulligan detail wrapper]',e)}
  document.addEventListener('keydown',function(e){if(e.key==='Escape')closeDetail()});
  setTimeout(bind,50);setTimeout(bind,400);
  console.info('Palworld OCG v0.7.87 mulligan card-tap detail enabled');
})();
</script>

<script id="v0784NegativeDamageGuardScript">
(()=>{
  ...
  function allZoneCards(){
    ...
    try{
      ...
      for(const side of ['p','a']){
        const pl=G[side];if(!pl)continue;
        for(const zone of ['deck','hand','pals','supports','grave','exile']){
          const arr=pl[zone];if(!Array.isArray(arr))continue;
          for(const c of arr)if(c&&typeof c==='object')out.push({side,zone,pl,c});
        }
      }
    }catch(_e){}
    return out;
  }
  function negativeCards(){return allZoneCards().filter(x=>Number.isFinite(Number(x.c.damage))&&Number(x.c.damage)<0)}
  function recentLogs(){try{return Array.isArray(G?.logs)?G.logs.slice(0,10):[]}catch(_e){return []}}
  function gameMeta(){
    try{return {mode:globalThis.v060?.mode||'solo',role:globalThis.v060?.role||'',turn:G?.turn??null,turnSeq:G?.turnSeq??null,phase:G?.phase||'',recentLogs:recentLogs()}}
    catch(_e){return {mode:'unknown'}}
  }
  function argLite(a){
    if(a==null||typeof a!=='object')return a;
    if(Array.isArray(a))return '[array '+a.length+']';
    return {no:a.no||'',name:a.name||'',uid:a.uid??null,damage:Number.isFinite(Number(a.damage))?Number(a.damage):a.damage??null,type:a.type||'',kind:a.kind||''};
  }
  function diag(stage,x,bad,extra){
    const now=Date.now(),sig=[stage,x.side,x.zone,x.c?.uid,x.c?.no,bad].join('|');
    if(sig===lastSig&&now-lastAt<1200)return;lastSig=sig;lastAt=now;
    const meta={op:'negativeDamageGuard',version:VERSION,stage,side:x.side,zone:x.zone,card:{no:x.c?.no||'',name:x.c?.name||'',uid:x.c?.uid??null},badDamage:bad,repairedDamage:0,...gameMeta(),...(extra||{})};
    try{
      if(typeof globalThis.palDiagRecord==='function')globalThis.palDiagRecord('ERR-DAMAGE-ORIGIN-0784','負のDamageを検出し0へ自動修復: '+(x.c?.no||x.c?.name||'card')+' '+bad,meta);
      else console.error('[ERR-DAMAGE-ORIGIN-0784]',meta);
    }catch(_e){console.error('[ERR-DAMAGE-ORIGIN-0784]',meta)}
  }
  function repair(stage,extra){
    if(busy)return 0;busy=true;let n=0;
    try{
      for(const x of negativeCards()){
        const bad=Number(x.c.damage);diag(stage,x,bad,extra);x.c.damage=0;n++;
      }
    }finally{busy=false}
    return n;
  }
  globalThis.v0784RepairNegativeDamage=repair;

  function wrap(name){
    try{
      const old=globalThis[name];if(typeof old!=='function'||old.__v0784NegWrapped)return false;
      const w=function(){
        const args=[...arguments];let r;
        try{r=old.apply(this,args)}catch(e){repair(name+':throw',{args:args.slice(0,4).map(argLite),error:String(e?.message||e)});throw e}
        const after=()=>repair(name,{args:args.slice(0,4).map(argLite)});
        try{
          if(r&&typeof r.then==='function')return Promise.resolve(r).then(v=>{after();return v},e=>{after();throw e});
          after();
        }catch(_e){}
        return r;
      };
      w.__v0784NegWrapped=true;w.__v0784NegOld=old;globalThis[name]=w;return true;
    }catch(_e){return false}
  }

  function installRemoteBoundaries(){
    try{
      const old=globalThis.v060UnpackCard;
      if(typeof old==='function'&&!old.__v0784DamageBoundary){
        const w=function(o){
          const r=old.apply(this,arguments);
          if(r&&Number.isFinite(Number(r.damage))&&Number(r.damage)<0){
            const bad=Number(r.damage);
            const fake={side:'remote',zone:'unpack',c:r};diag('v060UnpackCard',fake,bad,{rawDamage:o?.damage??null});r.damage=0;
          }
          return r;
        };w.__v0784DamageBoundary=true;w.__v0784NegOld=old;globalThis.v060UnpackCard=w;
      }
    }catch(_e){}
    try{
      const old=globalThis.v060PackCard;
      if(typeof old==='function'&&!old.__v0784DamageBoundary){
        const w=function(c){const r=old.apply(this,arguments);if(r&&Number.isFinite(Number(r.damage))&&Number(r.damage)<0)r.damage=0;return r};
        w.__v0784DamageBoundary=true;w.__v0784NegOld=old;globalThis.v060PackCard=w;
      }
    }catch(_e){}
    try{
      const old=globalThis.v060ApplyPackedState;
      if(typeof old==='function'&&!old.__v0784DamageBoundary){
        const w=function(){const r=old.apply(this,arguments);repair('v060ApplyPackedState',{remoteImport:true});return r};
        w.__v0784DamageBoundary=true;w.__v0784NegOld=old;globalThis.v060ApplyPackedState=w;
      }
    }catch(_e){}
  }
  ...
  function cleanLegacy(){
    try{
      const key='palworld_diag_v0762',a=JSON.parse(localStorage.getItem(key)||'[]');if(!Array.isArray(a))return;
      const b=a.filter(e=>!(e&&e.code==='ERR-STATE-0002'&&/damage=-\d+/.test(String(e.message||''))));
      if(b.length!==a.length)localStorage.setItem(key,JSON.stringify(b));
    }catch(_e){}
  }

  const names=['dealPalDamage','resolveBattle','v050ResolveActionText','resolveOnDeploy','activateAbility','clearTemps','finishTurn','stateCheck','v0713FlushAutos','v060Handle'];
  function install(){
    installRemoteBoundaries();for(const n of names)wrap(n);
    // render is a final safety boundary: it catches mutations made inside delayed callbacks.
    wrap('render');repair('install');
  }
  cleanLegacy();install();setTimeout(install,300);setTimeout(install,1200);
  addEventListener('pageshow',()=>setTimeout(install,80),{passive:true});
  console.info('Palworld OCG v0.7.87 negative Damage origin trace + self-repair enabled');
})();
</script>


<style id="v0785CardDetailDismissStyle">
/* v0.7.87 — every old showCardInfo modal gets a permanently reachable close control. */
...
(()=>{
  ...
  console.info('Palworld OCG v0.7.87 card-detail dismiss fix enabled');
})();
</script>

<style id="v0787QuickStableFinalStyle">
/* Final override: legacy v0748/v0754 sheets must not resize the active interrupt window. */
.modal.v0745CombatModal.v0787QuickModal{overflow-anchor:none!important;overscroll-behavior:none!important}
.modal.v0745CombatModal.v0787QuickModal>.modalCard{
  width:min(470px,76vw)!important;max-width:470px!important;
  height:220px!important;min-height:220px!important;max-height:220px!important;
  overflow:hidden!important;display:flex!important;flex-direction:column!important;
  contain:layout paint style!important;overflow-anchor:none!important;
}
.modal.v0745CombatModal.v0787QuickModal .v0787QuickList{
  display:flex!important;flex:1 1 auto!important;min-height:0!important;overflow-x:auto!important;overflow-y:hidden!important;
  gap:6px!important;align-items:stretch!important;scroll-snap-type:x proximity!important;overflow-anchor:none!important;
}
.modal.v0745CombatModal.v0787QuickModal .v0745QuickRow{
  flex:0 0 270px!important;width:270px!important;min-width:270px!important;
  height:116px!important;min-height:116px!important;max-height:116px!important;overflow:hidden!important;
}
.modal.v0745CombatModal.v0787QuickModal .v0745CombatActions{position:static!important;flex:0 0 auto!important}
@media(max-height:520px) and (orientation:landscape){
  .modal.v0745CombatModal.v0787QuickModal>.modalCard{width:min(460px,74vw)!important;max-width:460px!important;height:178px!important;min-height:178px!important;max-height:178px!important}
  .modal.v0745CombatModal.v0787QuickModal .v0745QuickRow{flex-basis:248px!important;width:248px!important;min-width:248px!important;height:92px!important;min-height:92px!important;max-height:92px!important}
}
</style>
...
function v0781ApplyCoreSource(html){
  const status={AI_TURN_LEAD:true};
  const gateCode="if(typeof v0781GateCpuAttack==='function'&&v0781GateCpuAttack())return;";
...
export default{
  async fetch(request){
    ...
    try{
      ...
      h.set("x-palworld-bridge","v0.7.87-stable-battle-visuals");
      ...
      if(u.pathname==="/"||u.pathname==="/index.html"){
        ...
        html=html.replace('\\n</head>','\n</head>');
        const core=v0781ApplyCoreSource(html);html=core.html;
      ...
      if(u.pathname==="/manifest.webmanifest"){
        ...
        m.description="Palworld OCG v0.7.87 — 安定化・テストプレイ・BP01同期待ちTier誤判定修正・戦闘表示整理・DAMAGE CHECK画像・攻撃矢印・CPU可読速度・Tier合法性ガード・全検査・診断";
        m.start_url="/?pwa=1&v=0787";
        m.scope="/";
      ...
      if(u.pathname==="/sw.js"){
        let sw=await r.text();
        sw+="\n// v0.7.87 stable serialized attack cue + test play + tier BP01 sync defer + direct damage art + diagnostics\n";
        h.delete("content-length");
        h.delete("content-encoding");
        h.delete("etag");
