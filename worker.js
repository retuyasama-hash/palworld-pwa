...
(()=>{
  ...
  function sync(){
    queued=false;
    try{
      const title=document.querySelector('.v04Title');if(title&&title.textContent!=='v0.7.70')title.textContent='v0.7.70';
...
/* v0.7.61 — show the revealed DAMAGE CHECK card as a large image like the user's reference. */
.v0758DamageReveal{
...
.v0758DamageReveal[hidden]{display:none!important}
.v0758DamageRevealPanel{
  width:min(420px,82vw)!important;max-width:420px!important;
  background:linear-gradient(180deg,rgba(20,20,28,.97),rgba(9,9,14,.96))!important;
  border:1px solid rgba(242,209,101,.9)!important;border-radius:18px!important;
  box-shadow:0 16px 38px rgba(0,0,0,.48),0 0 0 1px rgba(255,255,255,.05) inset!important;
  padding:14px 16px!important;color:#fff!important;text-align:center!important;
}
.v0758DamageRevealTitle{
  font-size:15px!important;font-weight:900!important;letter-spacing:.04em!important;color:#fff!important;
}
.v0758DamageRevealSub{
  margin-top:4px!important;font-size:10px!important;line-height:1.35!important;color:#ebedf5!important;opacity:.95!important;
}
.v0758DamageRevealCardWrap{
  margin:12px auto 10px!important;display:flex!important;justify-content:center!important;align-items:center!important;
}
.v0758DamageRevealCard{
  width:138px!important;max-width:138px!important;height:auto!important;transform:scale(1.02)!important;transform-origin:center center!important;
  box-shadow:0 10px 24px rgba(0,0,0,.36)!important;
}
.v0758DamageRevealCard .infoBtn,
.v0758DamageRevealCard button,
.v0758DamageRevealCard .v0754AssignOnPal,
.v0758DamageRevealCard .v0754AssignOnSource{display:none!important}
.v0758DamageRevealFoot{
  font-size:9px!important;line-height:1.2!important;color:#f2d56f!important;font-weight:800!important;
}
@media (max-height:520px) and (orientation:landscape){
  .v0758DamageRevealPanel{width:min(360px,78vw)!important;padding:11px 13px!important}
  .v0758DamageRevealTitle{font-size:13px!important}
  .v0758DamageRevealSub{font-size:8px!important}
  .v0758DamageRevealCard{width:112px!important;max-width:112px!important}
}
</style>
<script id="v0758DamageRevealScript">
(()=>{
  if(globalThis.__v0758DamageRevealApplied)return;
  globalThis.__v0758DamageRevealApplied=true;
  let seen='',timer=0,queued=false;

  function norm(v){return String(v==null?'':v).replace(/<[^>]*>/g,'').replace(/\s+/g,' ').trim()}
  function esc(s){return String(s).replace(/[&<>\"]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;"}[m]))}
  function findPanel(){
    try{
      const play=document.querySelector('.v04Play')||document.body;
      const nodes=[...play.querySelectorAll('.v0754DamageCheckPanel, div, section, aside, main')];
      for(const el of nodes){
        const text=norm(el.textContent||'');
        if(!/DAMAGE CHECK/i.test(text))continue;
  ...
  function buildSig(panel){
    const text=norm(panel?.textContent||'');
    const name=norm(panel?.querySelector('.card .name')?.textContent||'');
    return text+'|'+name;
  }
  function makeCardHtml(card){
    try{
      const clone=card.cloneNode(true);
      clone.classList.add('v0758DamageRevealCard');
      clone.querySelectorAll('button,.infoBtn,.v0754AssignOnPal,.v0754AssignOnSource').forEach(x=>x.remove());
      clone.removeAttribute('onclick');clone.removeAttribute('onpointerup');clone.removeAttribute('onpointerdown');
      return clone.outerHTML;
    }catch(_e){return ''}
  }
  function parseText(panel){
    const raw=norm(panel.textContent||'');
    let sub=raw.replace(/DAMAGE CHECK/ig,'').trim();
    sub=sub.replace(/^[:：\-\s]+/,'').trim();
    if(!sub)sub='ダメージでめくれたカード';
    return sub;
  }
  function ensure(){
    let el=document.getElementById('v0758DamageReveal');
    if(el)return el;
    el=document.createElement('div');el.id='v0758DamageReveal';el.className='v0758DamageReveal';el.hidden=true;
    document.body.appendChild(el);return el;
  }
  function hide(){const el=document.getElementById('v0758DamageReveal');if(el)el.hidden=true}
  function cpuMode(){
    try{const t=(document.getElementById('v0756CpuSpeedBtn')?.textContent||'').trim();if(t.includes('即時'))return 'instant';if(t.includes('×2'))return 'fast'}catch(_e){}
    return 'normal';
  }
  function show(panel){
    const card=panel.querySelector('.card'); if(!card)return;
    const name=norm(card.querySelector('.name')?.textContent||'');
    const sub=parseText(panel);
    const el=ensure();
    el.innerHTML='<div class="v0758DamageRevealPanel"><div class="v0758DamageRevealTitle">ダメージチェック</div><div class="v0758DamageRevealSub">'+esc(sub)+'</div><div class="v0758DamageRevealCardWrap">'+makeCardHtml(card)+'</div><div class="v0758DamageRevealFoot">'+esc(name||'カードを公開')+'</div></div>';
    el.hidden=false;
    clearTimeout(timer);
    const m=cpuMode();
    const dur=m==='instant'?900:(m==='fast'?1200:1650);
  ...
  function scan(){
    ...
    try{
      ...
      if(!panel)return;
      const sig=buildSig(panel);
      if(!sig||sig===seen)return;
      seen=sig;
      show(panel);
      setTimeout(()=>{if(seen===sig)seen=''},700);
    }catch(_e){}
  }
  ...
  console.info('Palworld OCG v0.7.61 damage reveal popup applied');
})();
</script>


<style id="v0759DamageRevealFixStyle">
/* v0.7.61 — DAMAGE CHECK result card is reconstructed from CARD_DB when the strip contains only text. */
.v0759DamageReveal{
...
.v0759DamageReveal[hidden]{display:none!important}
.v0759DamageRevealBox{
  width:min(390px,78vw)!important;max-width:390px!important;
  background:linear-gradient(180deg,rgba(17,18,25,.98),rgba(7,8,12,.97))!important;
  border:1px solid #e7cb6f!important;border-radius:18px!important;
  padding:12px 16px 13px!important;text-align:center!important;color:#fff!important;
  box-shadow:0 18px 42px rgba(0,0,0,.52),0 0 0 1px rgba(255,255,255,.05) inset!important;
}
.v0759DamageRevealHead{font-size:14px!important;line-height:1.15!important;font-weight:950!important;letter-spacing:.08em!important;color:#fff!important}
.v0759DamageRevealWho{margin-top:4px!important;font-size:9px!important;line-height:1.25!important;color:#cfd4df!important}
.v0759DamageCardStage{display:flex!important;justify-content:center!important;align-items:center!important;margin:10px auto 8px!important;min-height:148px!important}
.v0759DamageCardStage .card{
  ...
  margin:0!important;transform:scale(1.03)!important;transform-origin:center!important;
  box-shadow:0 12px 28px rgba(0,0,0,.42)!important;pointer-events:none!important;
}
.v0759DamageCardStage .card .name{font-size:8px!important;line-height:1.05!important}
.v0759DamageCardStage .card .stats,.v0759DamageCardStage .card .no{font-size:6.5px!important}
.v0759DamageCardStage .card button,.v0759DamageCardStage .card .infoBtn{display:none!important}
.v0759DamageRevealName{font-size:12px!important;line-height:1.25!important;font-weight:900!important;color:#fff!important}
.v0759DamageRevealEffect{margin-top:3px!important;font-size:9px!important;line-height:1.2!important;color:#f0d46f!important;font-weight:800!important}
@media (max-height:520px) and (orientation:landscape){
  .v0759DamageRevealBox{width:min(350px,72vw)!important;padding:9px 13px 10px!important}
  .v0759DamageRevealHead{font-size:12px!important}
  .v0759DamageCardStage{margin:7px auto 5px!important;min-height:122px!important}
  .v0759DamageCardStage .card{width:94px!important;min-width:94px!important;max-width:94px!important;height:126px!important;min-height:126px!important;max-height:126px!important}
  .v0759DamageRevealName{font-size:10px!important}
  .v0759DamageRevealEffect{font-size:7.5px!important}
}
</style>
<script id="v0759DamageRevealFixScript">
(()=>{
  if(globalThis.__v0759DamageRevealFixApplied)return;
  globalThis.__v0759DamageRevealFixApplied=true;
  let queued=false,lastPanel=null,lastCardNo='',lastShow=0,timer=0;

  function norm(v){return String(v==null?'':v).replace(/<[^>]*>/g,'').replace(/\s+/g,' ').trim()}
  function esc(v){return String(v==null?'':v).replace(/[&<>\"]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;"}[m]))}
  function getDamagePanel(){
    try{
      const root=document.querySelector('.v04Play')||document;
      const marked=[...root.querySelectorAll('.v0754DamageCheckPanel')].filter(x=>/DAMAGE CHECK/i.test(norm(x.textContent)));
      if(marked.length)return marked[0];
      const nodes=[...root.querySelectorAll('div,section,aside,main')];
      let best=null,bestArea=Infinity;
      for(const el of nodes){
        if(!/DAMAGE CHECK/i.test(norm(el.textContent)))continue;
        const r=el.getBoundingClientRect();
        if(!r.width||!r.height||r.height<24)continue;
  ...
  function cardDefs(){
    try{return (typeof CARD_DB!=='undefined'&&CARD_DB)?Object.values(CARD_DB):[]}catch(_e){return []}
  }
  function findDef(text){
    text=norm(text);
    if(!text)return null;
    const defs=cardDefs().filter(c=>c&&c.name);
    let best=null,bestLen=0;
    for(const c of defs){
      const n=norm(c.name);
      if(n&&text.includes(n)&&n.length>bestLen){best=c;bestLen=n.length}
    }
    if(best)return best;
    /* Some UI lines shorten the prefix before a dash. Match the longest useful name part. */
    for(const c of defs){
      const parts=norm(c.name).split(/[\-–—]/).map(x=>x.trim()).filter(x=>x.length>=4);
      for(const part of parts){
        if(text.includes(part)&&part.length>bestLen){best=c;bestLen=part.length}
      }
    }
    return best;
  }
  function fresh(def){
    try{if(typeof v071Fresh==='function'&&def&&def.no)return v071Fresh(def.no)}catch(_e){}
    try{return Object.assign({uid:-759001},def||{})}catch(_e){return def}
  }
  function renderCard(def){
    try{
      if(typeof cardHTML==='function'){
        const c=fresh(def);
        const html=cardHTML(c,{select:false});
  ...
  function ensure(){
    let el=document.getElementById('v0759DamageReveal');
    if(el)return el;
    el=document.createElement('div');el.id='v0759DamageReveal';el.className='v0759DamageReveal';el.hidden=true;
    document.body.appendChild(el);return el;
  }
  function hide(){const el=document.getElementById('v0759DamageReveal');if(el)el.hidden=true}
  function speedDuration(){
    try{
      const t=norm(document.getElementById('v0756CpuSpeedBtn')?.textContent||'');
      if(t.includes('即時'))return 1050;
      if(t.includes('×2'))return 1350;
    }catch(_e){}
    return 1750;
  }
  function isLucky(panelText){return /LUCKY|ラッキー|★\s*LUCKY/i.test(panelText)}
  function show(def,panelText){
    const html=renderCard(def);if(!html)return false;
    /* Hide the failed v0.7.58 overlay if it ever appears at the same time. */
    try{const old=document.getElementById('v0758DamageReveal');if(old)old.hidden=true}catch(_e){}
    const el=ensure();
    const who=/相手|OPPONENT|CPU/i.test(panelText)?'相手が受けたダメージ':'ダメージで公開されたカード';
    const effect=isLucky(panelText)?'★ LUCKY!':'ダメージチェック結果';
    el.innerHTML='<div class="v0759DamageRevealBox"><div class="v0759DamageRevealHead">DAMAGE CHECK</div><div class="v0759DamageRevealWho">'+esc(who)+'</div><div class="v0759DamageCardStage">'+html+'</div><div class="v0759DamageRevealName">'+esc(def.name||'公開カード')+'</div><div class="v0759DamageRevealEffect">'+esc(effect)+'</div></div>';
    el.hidden=false;
    clearTimeout(timer);timer=setTimeout(hide,speedDuration());
    return true;
  }
  function scan(){
    queued=false;
    try{
      const panel=getDamagePanel();
      if(!panel){lastPanel=null;lastCardNo='';return}
      const txt=norm(panel.textContent||'');
      const def=findDef(txt);
      /* Initial generic DAMAGE CHECK text has no card name. Wait for result text mutation. */
      if(!def)return;
      const no=String(def.no||def.name||'');
      const now=Date.now();
      const newPanel=panel!==lastPanel;
      if(!newPanel&&no===lastCardNo&&now-lastShow<900)return;
      if(show(def,txt)){
        lastPanel=panel;lastCardNo=no;lastShow=now;
      }
    }catch(_e){}
  }
  function schedule(){if(queued)return;queued=true;requestAnimationFrame(scan)}
  ...
  console.info('Palworld OCG v0.7.61 DAMAGE CHECK CARD_DB reveal applied');
})();
</script>


<style id="v0760DamageAndHandFixStyle">
/* v0.7.61 — robust DAMAGE CHECK card reveal + combat hand drawer. */
.v0760DamageReveal{
...
(()=>{
  ...
  console.info('Palworld OCG v0.7.61 robust DAMAGE CHECK reveal + combat hand drawer applied');
})();
</script>


<style id="v0761DirectDamageRevealStyle">
/* v0.7.61 — exact DAMAGE CHECK reveal, driven by playerDamage() itself. */
.v0761DamageReveal{
  position:fixed!important;inset:0!important;z-index:2147483400!important;
  display:flex!important;align-items:center!important;justify-content:center!important;
  padding:10px!important;pointer-events:none!important;background:rgba(0,0,0,.18)!important;
}
.v0761DamageReveal[hidden]{display:none!important}
.v0761DamageBox{
  width:min(420px,80vw)!important;max-width:420px!important;
  background:linear-gradient(180deg,rgba(17,20,27,.985),rgba(7,9,13,.98))!important;
  border:2px solid rgba(244,210,92,.96)!important;border-radius:18px!important;
  box-shadow:0 18px 46px rgba(0,0,0,.58),0 0 26px rgba(244,210,92,.14)!important;
  padding:12px 15px 14px!important;text-align:center!important;color:#fff!important;
}
.v0761DamageHead{font-size:14px!important;font-weight:950!important;letter-spacing:.08em!important}
.v0761DamageSide{margin-top:3px!important;font-size:9px!important;color:#d5dae4!important}
.v0761DamageCount{margin-top:2px!important;font-size:8px!important;color:#aeb7c7!important}
.v0761DamageImageStage{
  min-height:185px!important;margin:9px auto 7px!important;display:flex!important;align-items:center!important;justify-content:center!important;
}
.v0761DamageImageStage img.v0761DamageImg{
  display:block!important;width:132px!important;max-width:132px!important;height:auto!important;max-height:185px!important;
  object-fit:contain!important;border-radius:8px!important;box-shadow:0 11px 28px rgba(0,0,0,.48)!important;
}
.v0761DamageImageStage .card{
  ...
  margin:0!important;transform:none!important;box-shadow:0 11px 28px rgba(0,0,0,.48)!important;
}
.v0761DamageImageStage .card .name{font-size:9px!important}.v0761DamageImageStage .card .stats,.v0761DamageImageStage .card .no{font-size:6px!important}
.v0761DamageImageStage .card button,.v0761DamageImageStage .card .infoBtn{display:none!important}
.v0761DamageName{font-size:12px!important;line-height:1.25!important;font-weight:950!important}
.v0761DamageLucky{margin-top:4px!important;font-size:12px!important;font-weight:950!important;color:#f5d85d!important;text-shadow:0 0 12px rgba(245,216,93,.35)!important}
.v0761DamageNormal{margin-top:4px!important;font-size:9px!important;font-weight:850!important;color:#d8dde7!important}
@media(max-height:520px) and (orientation:landscape){
  .v0761DamageBox{width:min(360px,72vw)!important;padding:8px 12px 10px!important}
  .v0761DamageImageStage{min-height:126px!important;margin:5px auto!important}
  .v0761DamageImageStage img.v0761DamageImg{width:94px!important;max-width:94px!important;max-height:126px!important}
  .v0761DamageImageStage .card{width:94px!important;min-width:94px!important;max-width:94px!important;height:125px!important;min-height:125px!important;max-height:125px!important}
  .v0761DamageHead{font-size:12px!important}.v0761DamageName{font-size:10px!important}.v0761DamageLucky{font-size:10px!important}
}
</style>
<script id="v0761DirectDamageRevealScript">
(()=>{
  if(globalThis.__v0761DirectDamageRevealApplied)return;
  globalThis.__v0761DirectDamageRevealApplied=true;
  let queue=[],showing=false,timer=0;

  function norm(v){return String(v==null?'':v).replace(/\s+/g,' ').trim()}
  function esc(v){return String(v==null?'':v).replace(/[&<>\"]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;"}[m]))}
  function speed(){
    try{const t=norm(document.getElementById('v0756CpuSpeedBtn')?.textContent||'');if(t.includes('即時'))return 520;if(t.includes('×2'))return 760}catch(_e){}
    return 1150;
  }
  function host(){
    let el=document.getElementById('v0761DamageReveal');
    if(!el){el=document.createElement('div');el.id='v0761DamageReveal';el.className='v0761DamageReveal';el.hidden=true;document.body.appendChild(el)}
    return el;
  }
  function imageMarkup(c){
    try{
      if(typeof imageTag==='function'){
        const h=imageTag(c,'v0761DamageImg');
        if(h)return h;
      }
    }catch(_e){}
    try{
      if(typeof cardHTML==='function'){
        const tmp=document.createElement('div');tmp.innerHTML=cardHTML(c,{select:false});
        const card=tmp.querySelector('.card')||tmp.firstElementChild;
        if(card){
          card.removeAttribute('onclick');card.removeAttribute('onpointerup');card.removeAttribute('onpointerdown');
          card.querySelectorAll('button,.infoBtn,.v0754AssignOnPal,.v0754AssignOnSource').forEach(x=>x.remove());
    ...
    return '<div style="width:132px;height:176px;border:1px solid #a98e3e;border-radius:8px;display:flex;align-items:center;justify-content:center;padding:8px;background:#151922;color:#fff;font-size:10px">'+esc(c?.name||'公開カード')+'</div>';
  }
  function hideOld(){
    try{['v0758DamageReveal','v0759DamageReveal','v0760DamageReveal'].forEach(id=>{const x=document.getElementById(id);if(x)x.hidden=true})}catch(_e){}
  }
  function render(item){
    hideOld();
    const el=host();
    const c=item.card||{};
    const side=item.defName?item.defName+' のダメージチェック':'ダメージチェック';
    const count=item.total>1?(item.index+' / '+item.total+'枚目'):'';
    el.innerHTML='<div class="v0761DamageBox">'+
      '<div class="v0761DamageHead">DAMAGE CHECK</div>'+
      '<div class="v0761DamageSide">'+esc(side)+'</div>'+
      (count?'<div class="v0761DamageCount">'+esc(count)+'</div>':'')+
      '<div class="v0761DamageImageStage">'+imageMarkup(c)+'</div>'+
      '<div class="v0761DamageName">'+esc(c.name||c.no||'公開カード')+'</div>'+
      (c.lucky?'<div class="v0761DamageLucky">★ LUCKY!</div>':'<div class="v0761DamageNormal">公開</div>')+
      '</div>';
    el.hidden=false;
  }
  function pump(){
    if(showing||!queue.length)return;
    showing=true;const item=queue.shift();render(item);
    clearTimeout(timer);timer=setTimeout(()=>{const el=document.getElementById('v0761DamageReveal');if(el)el.hidden=true;showing=false;pump()},speed());
  }
  function enqueue(cards,def){
    if(!cards||!cards.length)return;
    cards.forEach((c,i)=>queue.push({card:c,defName:def?.name||'',index:i+1,total:cards.length}));
    if(queue.length>12)queue=queue.slice(-12);
    pump();
  }

  /* Exact source of truth: capture the actual top-deck cards BEFORE the engine moves them. */
  try{
    if(typeof playerDamage==='function'&&!playerDamage.__v0761DirectWrapped){
      const old=playerDamage;
      const wrapped=function(def,amount){
        let revealed=[];
        try{
          const n=Math.max(0,Number(amount)||0);
          const deck=Array.isArray(def?.deck)?def.deck:[];
          for(let i=0;i<n&&i<deck.length;i++){
            const c=deck[i];if(!c)continue;
            revealed.push(c);
            if(c.lucky)break;
          }
        }catch(_e){}
        const r=old.apply(this,arguments);
        try{enqueue(revealed,def)}catch(_e){}
        return r;
      };
      wrapped.__v0761DirectWrapped=true;wrapped.__v0761DirectOld=old;playerDamage=wrapped;
    }
  }catch(_e){console.warn('v0.7.61 direct playerDamage reveal hook failed',_e)}

  addEventListener('pageshow',()=>{queue.length=0;showing=false;clearTimeout(timer);const el=document.getElementById('v0761DamageReveal');if(el)el.hidden=true},{passive:true});
  console.info('Palworld OCG v0.7.61 direct playerDamage card reveal applied');
})();
</script>
...
.v0766DamageHead{font-size:15px!important;font-weight:950!important;letter-spacing:.09em!important}
.v0766DamageCount{margin-top:3px!important;font-size:8px!important;color:#b9c3d0!important}
.v0766DamageStage{height:190px!important;margin:7px auto 6px!important;display:flex!important;align-items:center!important;justify-content:center!important;overflow:visible!important}
.v0766DamageStage img{display:block!important;width:138px!important;max-width:138px!important;max-height:190px!important;height:auto!important;object-fit:contain!important;border-radius:8px!important;box-shadow:0 12px 30px rgba(0,0,0,.58)!important}
.v0766DamageStage .card{width:128px!important;min-width:128px!important;max-width:128px!important;height:auto!important;min-height:170px!important;max-height:190px!important;transform:none!important;margin:0!important;pointer-events:none!important}
.v0766DamageStage .card button,.v0766DamageStage .card .infoBtn{display:none!important}
.v0766DamageFallback{width:136px!important;height:181px!important;border:1px solid #9c8537!important;border-radius:8px!important;background:#101821!important;display:flex!important;align-items:center!important;justify-content:center!important;padding:9px!important;font-size:10px!important;font-weight:900!important;line-height:1.35!important}
.v0766DamageName{font-size:12px!important;line-height:1.25!important;font-weight:950!important}
.v0766DamageLucky{margin-top:4px!important;font-size:13px!important;font-weight:950!important;color:#f5d85d!important;text-shadow:0 0 12px rgba(245,216,93,.45)!important}
.v0766DamageNormal{margin-top:4px!important;font-size:9px!important;font-weight:850!important;color:#dce2ea!important}
@media(max-height:520px) and (orientation:landscape){.v0766DamageBox{width:min(355px,70vw)!important;padding:7px 10px 8px!important}.v0766DamageStage{height:128px!important;margin:4px auto!important}.v0766DamageStage img{width:96px!important;max-width:96px!important;max-height:128px!important}.v0766DamageStage .card{width:92px!important;min-width:92px!important;max-width:92px!important;min-height:120px!important;max-height:128px!important}.v0766DamageFallback{width:96px!important;height:126px!important;font-size:8px!important}.v0766DamageHead{font-size:12px!important}.v0766DamageName{font-size:10px!important}.v0766DamageLucky{font-size:10px!important}}
</style>
<script id="v0766DamageDomRevealScript">
(()=>{
  if(globalThis.__v0766DamageDomRevealApplied)return;
  globalThis.__v0766DamageDomRevealApplied=true;
  let scheduled=false,lastSig='',lastAt=0,timer=0,failedSig='';
  const esc=v=>String(v==null?'':v).replace(/[&<>\"]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;"}[m]));
  const flat=v=>String(v==null?'':v).replace(/\s+/g,' ').trim();
  const key=v=>flat(v).normalize('NFKC').replace(/[\s\-‐‑‒–—―−ー－・:：!！★☆]/g,'').toLowerCase();
  function defs(){try{return (typeof CARD_DB!=='undefined'&&CARD_DB)?Object.values(CARD_DB):[]}catch(_e){return []}}
  function findDef(text){
    const tk=key(text);if(!tk)return null;let best=null,bestLen=0;
    for(const c of defs()){
      if(!c||!c.name)continue;const nk=key(c.name);if(nk.length<3)continue;
      if(tk.includes(nk)&&nk.length>bestLen){best=c;bestLen=nk.length}
    }
    return best;
  }
  function visible(el){try{const r=el.getBoundingClientRect();const s=getComputedStyle(el);return r.width>20&&r.height>20&&s.display!=='none'&&s.visibility!=='hidden'&&Number(s.opacity||1)>0}catch(_e){return false}}
  function findResult(){
    try{
      const root=document.querySelector('.v04Play')||document;
      const seen=new Set(),arr=[];
      root.querySelectorAll('.v0754DamageCheckPanel,div,section,aside,main').forEach(el=>{if(seen.has(el))return;seen.add(el);const txt=flat(el.textContent||'');if(!/DAMAGE\s*CHECK/i.test(txt)||txt.length>650||!visible(el))return;const def=findDef(txt);if(!def)return;const r=el.getBoundingClientRect();arr.push({el,txt,def,area:r.width*r.height})});
      arr.sort((a,b)=>a.area-b.area);
      return arr[0]||null;
    }catch(_e){return null}
  }
  function add(a,x){x=String(x||'').trim();if(x&&!a.includes(x))a.push(x)}
  function sources(c){
    const a=[];
    try{if(typeof v050ImageUrls!=='undefined'&&c?.no)add(a,v050ImageUrls[c.no])}catch(_e){}
    try{if(typeof imageCandidates==='function'&&c?.no)for(const x of imageCandidates(c.no)||[])add(a,x)}catch(_e){}
    try{if(typeof EMBEDDED_CARD_IMAGES!=='undefined'&&c?.no)add(a,EMBEDDED_CARD_IMAGES[c.no])}catch(_e){}
    add(a,c?.imageUrl);add(a,c?.imageFallbackUrl);
    if(c?.no){for(const ext of ['webp','png','jpg','jpeg','svg']){try{add(a,new URL('assets/cards/'+c.no+'.'+ext,location.href).href)}catch(_e){}}}
    try{
      if(typeof imageTag==='function'){
        const t=document.createElement('div');t.innerHTML=imageTag(c,'v0766Probe')||'';const im=t.querySelector('img');if(im){add(a,im.getAttribute('src'));add(a,im.src);const raw=im.getAttribute('data-candidates')||im.getAttribute('data-srcs');if(raw){try{for(const x of JSON.parse(raw.replace(/&quot;/g,'"')))add(a,x)}catch(_e){}}}
      }
    }catch(_e){}
    return a;
  }
  function cardFallback(c){
    try{
      if(typeof cardHTML==='function'){
        const obj=Object.assign({uid:-766001,rest:false,damage:0,tempPower:0,tempStrike:0},c||{});
        const t=document.createElement('div');t.innerHTML=cardHTML(obj,{select:false})||'';const card=t.querySelector('.card');
        if(card){card.removeAttribute('onclick');card.removeAttribute('onpointerup');card.removeAttribute('onpointerdown');card.querySelectorAll('button,.infoBtn').forEach(x=>x.remove());return card.outerHTML}
      }
    }catch(_e){}
    return '<div class="v0766DamageFallback">'+esc(c?.name||c?.no||'公開カード')+'</div>';
  }
  globalThis.v0766DamageImgError=function(img){
    try{
      const a=JSON.parse(img.getAttribute('data-v0766-srcs')||'[]');let i=(Number(img.getAttribute('data-v0766-i'))||0)+1;
      if(i<a.length){img.setAttribute('data-v0766-i',String(i));img.src=a[i];return}
      const name=img.alt||'公開カード';const no=img.getAttribute('data-no')||'';
      if(typeof palDiagRecord==='function')palDiagRecord('ERR-DMG-0105','Damage Check card image failed to load',{op:'damageRevealImage',no,name,sources:a.length},null,true);
      const fb=document.createElement('div');fb.className='v0766DamageFallback';fb.textContent=name;img.replaceWith(fb);
    }catch(_e){try{img.style.display='none'}catch(__){}}
  };
  function imageHtml(c){
    const a=sources(c);
    if(!a.length)return cardFallback(c);
    return '<img alt="'+esc(c?.name||'')+'" data-no="'+esc(c?.no||'')+'" src="'+esc(a[0])+'" data-v0766-srcs="'+esc(JSON.stringify(a))+'" data-v0766-i="0" onerror="v0766DamageImgError(this)">';
  }
  function host(){let el=document.getElementById('v0766DamageReveal');if(!el){el=document.createElement('div');el.id='v0766DamageReveal';el.className='v0766DamageReveal';el.hidden=true;document.body.appendChild(el)}return el}
  function hideOld(){try{['v0758DamageReveal','v0759DamageReveal','v0760DamageReveal','v0761DamageReveal','v0765DamageReveal'].forEach(id=>{const x=document.getElementById(id);if(x)x.hidden=true})}catch(_e){}}
  function duration(){try{const t=flat(document.getElementById('v0756CpuSpeedBtn')?.textContent||'');if(t.includes('即時'))return 1050;if(t.includes('×2'))return 1350}catch(_e){}return 1800}
  function show(hit){
    const c=hit.def,txt=hit.txt,el=host();hideOld();
    const m=txt.match(/(\d+)\s*\/\s*(\d+)\s*枚目/);const count=m?'<div class="v0766DamageCount">'+esc(m[1])+' / '+esc(m[2])+'枚目</div>':'';
    const lucky=/LUCKY|ラッキー/i.test(txt)||!!c.lucky;
    el.innerHTML='<div class="v0766DamageBox"><div class="v0766DamageHead">DAMAGE CHECK</div>'+count+'<div class="v0766DamageStage">'+imageHtml(c)+'</div><div class="v0766DamageName">'+esc(c.name||c.no||'公開カード')+'</div>'+(lucky?'<div class="v0766DamageLucky">★ LUCKY!</div>':'<div class="v0766DamageNormal">公開</div>')+'</div>';
    el.hidden=false;clearTimeout(timer);timer=setTimeout(()=>{el.hidden=true},duration());
  }
  function scan(){
    scheduled=false;const h=findResult();
    if(!h)return;
    const sig=String(h.def?.no||h.def?.name||'')+'|'+flat(h.txt).slice(0,240);const now=Date.now();
    if(sig===lastSig&&now-lastAt<1100)return;lastSig=sig;lastAt=now;show(h);
  }
...
<style id="v0768DamageGlobalRevealStyle">
.v0768DamageReveal{position:fixed!important;inset:0!important;z-index:2147483647!important;display:flex!important;align-items:center!important;justify-content:center!important;padding:10px!important;pointer-events:none!important;background:rgba(0,0,0,.34)!important}
.v0768DamageReveal[hidden]{display:none!important}
.v0768DamageBox{width:min(430px,80vw)!important;background:linear-gradient(180deg,rgba(7,17,18,.995),rgba(3,7,9,.995))!important;border:2px solid rgba(244,209,78,.98)!important;border-radius:18px!important;box-shadow:0 18px 58px rgba(0,0,0,.78),0 0 28px rgba(244,209,78,.20)!important;padding:10px 14px 12px!important;text-align:center!important;color:#fff!important}
.v0768DamageHead{font-size:15px!important;font-weight:950!important;letter-spacing:.09em!important}
.v0768DamageCount{margin-top:2px!important;font-size:8px!important;color:#c9d0d9!important}
.v0768DamageStage{height:190px!important;margin:7px auto 6px!important;display:flex!important;align-items:center!important;justify-content:center!important}
.v0768DamageStage img{display:block!important;width:140px!important;max-width:140px!important;max-height:190px!important;height:auto!important;object-fit:contain!important;border-radius:8px!important;box-shadow:0 12px 30px rgba(0,0,0,.62)!important}
.v0768DamageStage .card{width:130px!important;min-width:130px!important;max-width:130px!important;min-height:170px!important;max-height:190px!important;transform:none!important;margin:0!important;pointer-events:none!important}
.v0768DamageStage .card button,.v0768DamageStage .card .infoBtn{display:none!important}
.v0768DamageFallback{width:138px!important;height:182px!important;border:1px solid #a68d3d!important;border-radius:8px!important;background:#101820!important;display:flex!important;align-items:center!important;justify-content:center!important;padding:9px!important;font-size:10px!important;font-weight:900!important;line-height:1.35!important}
.v0768DamageName{font-size:12px!important;line-height:1.25!important;font-weight:950!important}
.v0768DamageLucky{margin-top:4px!important;font-size:13px!important;font-weight:950!important;color:#f5d85d!important;text-shadow:0 0 12px rgba(245,216,93,.45)!important}
.v0768DamageNormal{margin-top:4px!important;font-size:9px!important;font-weight:850!important;color:#dce2ea!important}
@media(max-height:520px) and (orientation:landscape){.v0768DamageBox{width:min(360px,70vw)!important;padding:7px 10px 8px!important}.v0768DamageStage{height:132px!important;margin:4px auto!important}.v0768DamageStage img{width:100px!important;max-width:100px!important;max-height:132px!important}.v0768DamageStage .card{width:96px!important;min-width:96px!important;max-width:96px!important;min-height:124px!important;max-height:132px!important}.v0768DamageFallback{width:100px!important;height:130px!important;font-size:8px!important}.v0768DamageHead{font-size:12px!important}.v0768DamageName{font-size:10px!important}.v0768DamageLucky{font-size:10px!important}}
</style>
<script id="v0768DamageGlobalRevealScript">
(()=>{
  if(globalThis.__v0768DamageGlobalRevealApplied)return;
  globalThis.__v0768DamageGlobalRevealApplied=true;
  let scheduled=false,lastSig='',lastAt=0,timer=0,lastMiss='';
  const esc=v=>String(v==null?'':v).replace(/[&<>\"]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;"}[m]));
  const flat=v=>String(v==null?'':v).replace(/\s+/g,' ').trim();
  const key=v=>flat(v).normalize('NFKC').replace(/[\s\-‐‑‒–—―−ー－・:：!！★☆]/g,'').toLowerCase();
  function visible(el){try{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return r.width>30&&r.height>20&&s.display!=='none'&&s.visibility!=='hidden'&&Number(s.opacity||1)>0}catch(_e){return false}}
  function isOwn(el){try{return !!el.closest('#v0758DamageReveal,#v0759DamageReveal,#v0760DamageReveal,#v0761DamageReveal,#v0765DamageReveal,#v0766DamageReveal,#v0767DamageReveal,#v0768DamageReveal')}catch(_e){return false}}
  function damagePanel(){
    const out=[];
    try{
      document.querySelectorAll('div,section,aside,main').forEach(el=>{
        if(isOwn(el)||!visible(el))return;
        const txt=flat(el.textContent||'');
        if(!/DAMAGE\s*CHECK/i.test(txt)||txt.length>500)return;
        const r=el.getBoundingClientRect();
        if(r.width<220||r.height<45)return;
        out.push({el,txt,area:r.width*r.height});
      });
  ...
  function allDefs(){
    ...
    const add=c=>{if(!c||typeof c!=='object')return;const id=String(c.no||'')+'|'+String(c.name||'');if(seen.has(id))return;seen.add(id);a.push(c)};
    try{Object.values(CARD_DB||{}).forEach(add)}catch(_e){}
    try{Object.values(BP01_BY_NO||{}).forEach(add)}catch(_e){}
    try{if(Array.isArray(BP01_CATALOG))BP01_CATALOG.forEach(add)}catch(_e){}
    try{for(const side of ['p','a']){const pl=G?.[side];if(!pl)continue;for(const z of ['grave','hand','deck','pals','supports','buildings','gears','exile']){const x=pl?.[z];if(Array.isArray(x))x.forEach(add)}}}catch(_e){}
    return a;
  }
  function resolve(txt){
    const tk=key(txt);let best=null,score=-1;
    for(const c of allDefs()){
      const nk=key(c?.name||'');if(nk.length<2||!tk.includes(nk))continue;
      const sc=nk.length*100+(c.no?1000:0);if(sc>score){score=sc;best=c}
    }
    return best;
  }
  function fallbackCard(c){
    try{if(typeof cardHTML==='function'){const obj=Object.assign({uid:-768001,rest:false,damage:0,tempPower:0,tempStrike:0},c||{});const t=document.createElement('div');t.innerHTML=cardHTML(obj,{select:false})||'';const card=t.querySelector('.card');if(card){card.removeAttribute('onclick');card.removeAttribute('onpointerup');card.removeAttribute('onpointerdown');card.querySelectorAll('button,.infoBtn').forEach(x=>x.remove());return card.outerHTML}}}catch(_e){}
    return '<div class="v0768DamageFallback">'+esc(c?.name||c?.no||'公開カード')+'</div>';
  }
  function imageMarkup(c){
    try{if(typeof imageTag==='function'){const h=imageTag(c,'v0768DamageImg');if(h)return h}}catch(_e){}
    return fallbackCard(c);
  }
  globalThis.v0768DamageImgError=function(img){
    try{
      let list=[];try{list=JSON.parse(img.dataset.srcs||'[]')}catch(_e){}
      let i=(Number(img.dataset.i)||0)+1;
      if(i<list.length){img.dataset.i=String(i);img.src=list[i];return}
      const name=img.alt||'公開カード';const c=resolve(name)||{name};const t=document.createElement('div');t.innerHTML=fallbackCard(c);img.replaceWith(t.firstElementChild||t);
      if(typeof palDiagRecord==='function')palDiagRecord('ERR-DMG-0109','Damage Check image candidates exhausted',{op:'v0768Image',name,count:list.length},null,true);
    }catch(_e){}
  };
  function host(){let el=document.getElementById('v0768DamageReveal');if(!el){el=document.createElement('div');el.id='v0768DamageReveal';el.className='v0768DamageReveal';el.hidden=true;document.body.appendChild(el)}return el}
  function hideOld(){try{['v0758DamageReveal','v0759DamageReveal','v0760DamageReveal','v0761DamageReveal','v0765DamageReveal','v0766DamageReveal','v0767DamageReveal'].forEach(id=>{const x=document.getElementById(id);if(x)x.hidden=true})}catch(_e){}}
  function duration(){try{const t=flat(document.getElementById('v0756CpuSpeedBtn')?.textContent||'');if(t.includes('即時'))return 1200;if(t.includes('×2'))return 1500}catch(_e){}return 2000}
  function show(hit,c){
    const el=host();hideOld();const txt=hit.txt;const m=txt.match(/(\d+)\s*\/\s*(\d+)\s*枚目/);const count=m?'<div class="v0768DamageCount">'+esc(m[1])+' / '+esc(m[2])+'枚目</div>':'';const lucky=/LUCKY|ラッキー/i.test(txt)||!!c?.lucky;
    let art=imageMarkup(c).replace(/onerror="v04ImgError\(this\)"/g,'onerror="v0768DamageImgError(this)"');
    el.innerHTML='<div class="v0768DamageBox"><div class="v0768DamageHead">DAMAGE CHECK</div>'+count+'<div class="v0768DamageStage">'+art+'</div><div class="v0768DamageName">'+esc(c?.name||c?.no||'公開カード')+'</div>'+(lucky?'<div class="v0768DamageLucky">★ LUCKY!</div>':'<div class="v0768DamageNormal">公開</div>')+'</div>';
    el.hidden=false;clearTimeout(timer);timer=setTimeout(()=>{el.hidden=true},duration());
  }
  function scan(){
    scheduled=false;if(globalThis.__v0769DamageInlineApplied)return;const hit=damagePanel();if(!hit)return;
    const c=resolve(hit.txt);
    if(!c){const miss=hit.txt.slice(0,220);if(miss!==lastMiss){lastMiss=miss;try{if(typeof palDiagRecord==='function')palDiagRecord('ERR-DMG-0108','Global Damage Check panel found but card definition could not be resolved',{op:'v0768Resolve',text:miss},null,true)}catch(_e){}}return}
    const sig=String(c.no||c.name)+'|'+hit.txt.slice(0,240),now=Date.now();if(sig===lastSig&&now-lastAt<1400)return;lastSig=sig;lastAt=now;show(hit,c);
  }
  function schedule(){if(scheduled)return;scheduled=true;requestAnimationFrame(scan)}
  try{new MutationObserver(schedule).observe(document.documentElement,{childList:true,subtree:true,characterData:true,attributes:true,attributeFilter:['class','hidden','style']})}catch(_e){}
  document.addEventListener('pointerup',schedule,{passive:true});setInterval(schedule,120);
  addEventListener('pageshow',()=>{lastSig='';lastAt=0;lastMiss='';const x=document.getElementById('v0768DamageReveal');if(x)x.hidden=true;setTimeout(schedule,50)},{passive:true});
  try{if(typeof render==='function'){const base=render;render=function(){const r=base.apply(this,arguments);try{const t=document.querySelector('.v04Title');if(t&&!G?.cpuVsCpu)t.textContent='v0.7.70'}catch(_e){}return r}}}catch(_e){}
  try{if(typeof showStart==='function'){const base=showStart;showStart=function(){const r=base.apply(this,arguments);setTimeout(()=>{try{const b=document.querySelector('h2 .badge.official');if(b)b.textContent='v0.7.70'}catch(_e){}},0);return r}}}catch(_e){}
  schedule();setTimeout(schedule,250);
  console.info('Palworld OCG v0.7.68 global DAMAGE CHECK detector applied');
})();
</script>

<style id="v0769BattleClarityStyle">
/* v0.7.70 — make DAMAGE CHECK, attack target and CPU pacing readable on a phone. */
#v0758DamageReveal,#v0759DamageReveal,#v0760DamageReveal,#v0761DamageReveal,#v0765DamageReveal,#v0766DamageReveal,#v0767DamageReveal,#v0768DamageReveal{display:none!important}
.v0754DamageCheckPanel.v0769DamageEnhanced{
  max-height:none!important;min-height:132px!important;height:auto!important;overflow:visible!important;
  display:grid!important;grid-template-columns:110px minmax(0,1fr)!important;grid-template-rows:auto 1fr!important;
  gap:4px 10px!important;align-items:center!important;padding:7px 10px!important;
  border:2px solid #e7c952!important;background:linear-gradient(180deg,#071714fa,#06100dfa)!important;
  box-shadow:0 10px 26px #0009!important;
}
.v0754DamageCheckPanel.v0769DamageEnhanced>:not(.v0769DamageArt):not(.v0769DamageInfo){display:none!important}
.v0754DamageCheckPanel.v0769DamageEnhanced>.v0769DamageArt{grid-row:1 / span 2;grid-column:1;display:flex;align-items:center;justify-content:center;min-height:118px!important}
.v0769DamageArt img{display:block!important;width:88px!important;max-width:88px!important;max-height:118px!important;height:auto!important;object-fit:contain!important;border-radius:7px!important;box-shadow:0 8px 22px #000b!important}
.v0769DamageArt .card{width:88px!important;min-width:88px!important;max-width:88px!important;height:118px!important;min-height:118px!important;max-height:118px!important;margin:0!important;transform:none!important;pointer-events:none!important}
.v0769DamageArt .card button,.v0769DamageArt .card .infoBtn{display:none!important}
.v0769DamageArtFallback{width:88px;height:118px;border-radius:7px;border:1px solid #c7aa46;background:#15221d;display:flex;align-items:center;justify-content:center;padding:7px;text-align:center;font-size:8px;font-weight:900;line-height:1.25;color:#fff}
.v0769DamageInfo{grid-column:2;min-width:0!important;align-self:center!important}
.v0769DamageLabel{font-size:10px!important;font-weight:950!important;letter-spacing:.1em!important;color:#f7e082!important;margin-bottom:3px!important}
.v0769DamageName{font-size:12px!important;line-height:1.25!important;font-weight:950!important;color:#fff!important}
.v0769DamageLucky{margin-top:4px!important;font-size:12px!important;font-weight:950!important;color:#ffd95d!important;text-shadow:0 0 10px #ffd95d66!important}
.v0769DamageCount{margin-top:3px!important;font-size:8px!important;color:#d4ddd8!important}

#v0769AttackLayer{position:fixed!important;inset:0!important;z-index:2147483200!important;pointer-events:none!important;overflow:hidden!important}
#v0769AttackLayer svg{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;overflow:visible!important}
.v0769AttackPath{fill:none!important;stroke:#ffd84f!important;stroke-width:8!important;stroke-linecap:round!important;filter:drop-shadow(0 0 4px #ff3d28) drop-shadow(0 0 10px #000)!important;stroke-dasharray:18 10!important;animation:v0769Dash .5s linear infinite!important}
@keyframes v0769Dash{to{stroke-dashoffset:-56}}
.v0769AttackRing{position:fixed!important;border-radius:12px!important;border:4px solid #ffcf45!important;box-shadow:0 0 0 3px #ef3f2fbb,0 0 22px #ff553dcc!important;animation:v0769Ring .55s ease-in-out infinite alternate!important}
.v0769AttackRing.target{border-color:#fff27c!important;box-shadow:0 0 0 3px #e52c2cdd,0 0 26px #ff3f36!important}
@keyframes v0769Ring{from{filter:brightness(.9)}to{filter:brightness(1.35)}}
.v0769AttackBadge{position:fixed!important;left:50%!important;top:12%!important;transform:translateX(-50%)!important;padding:7px 14px!important;border-radius:999px!important;background:#180b08f2!important;border:2px solid #ffcf45!important;color:#fff!important;font-size:11px!important;font-weight:950!important;box-shadow:0 7px 24px #000c!important;white-space:nowrap!important;max-width:82vw!important;overflow:hidden!important;text-overflow:ellipsis!important}
.v0769AttackBadge b{color:#ffd84f!important}

@media(max-height:520px) and (orientation:landscape){
  .v0754DamageCheckPanel.v0769DamageEnhanced{grid-template-columns:86px minmax(0,1fr)!important;min-height:106px!important;padding:5px 8px!important;gap:3px 8px!important}
  .v0754DamageCheckPanel.v0769DamageEnhanced>.v0769DamageArt{min-height:94px!important}
  .v0769DamageArt img{width:70px!important;max-width:70px!important;max-height:94px!important}
  .v0769DamageArt .card{width:70px!important;min-width:70px!important;max-width:70px!important;height:94px!important;min-height:94px!important;max-height:94px!important}
  .v0769DamageArtFallback{width:70px;height:94px;font-size:7px!important}
  .v0769DamageLabel{font-size:8px!important}.v0769DamageName{font-size:10px!important}.v0769DamageLucky{font-size:10px!important}
  .v0769AttackBadge{top:8%!important;font-size:9px!important;padding:5px 10px!important}
  .v0769AttackPath{stroke-width:6!important}
}
</style>
<script id="v0769BattleClarityScript">
(()=>{
  if(globalThis.__v0769BattleClarityApplied)return;
  globalThis.__v0769BattleClarityApplied=true;
  globalThis.__v0769DamageInlineApplied=true;
  let scheduled=false,lastDamageSig='',lastDamageAt=0,lastAttackSig='',lastAttackAt=0;
  let graveSeen=new Set(),recentGrave=[];
  const flat=v=>String(v==null?'':v).replace(/\s+/g,' ').trim();
  const norm=v=>flat(v).normalize('NFKC').replace(/[\s\-‐‑‒–—―−ー－・:：!！★☆]/g,'').toLowerCase();
  const esc=v=>String(v==null?'':v).replace(/[&<>\"]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;"}[m]));
  function visible(el){try{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return r.width>20&&r.height>14&&s.display!=='none'&&s.visibility!=='hidden'&&Number(s.opacity||1)>0}catch(_e){return false}}
  function allDefs(){
    const out=[],seen=new Set();const add=c=>{if(!c||typeof c!=='object')return;const k=String(c.no||'')+'|'+String(c.name||'');if(seen.has(k))return;seen.add(k);out.push(c)};
    try{Object.values(CARD_DB||{}).forEach(add)}catch(_e){}try{Object.values(BP01_BY_NO||{}).forEach(add)}catch(_e){}try{if(Array.isArray(BP01_CATALOG))BP01_CATALOG.forEach(add)}catch(_e){}
    try{for(const side of ['p','a']){const pl=G?.[side];if(!pl)continue;for(const z of ['grave','hand','deck','pals','supports','buildings','gears','exile']){const a=pl?.[z];if(Array.isArray(a))a.forEach(add)}}}catch(_e){}
    return out;
  }
  function updateGraves(){
    try{for(const side of ['p','a']){const g=G?.[side]?.grave;if(!Array.isArray(g))continue;for(const c of g){const id=String(c?.uid??(c?.no+'|'+c?.name));if(!graveSeen.has(id)){graveSeen.add(id);recentGrave.push({c,at:Date.now()});}}}recentGrave=recentGrave.filter(x=>Date.now()-x.at<3500).slice(-8)}catch(_e){}
  }
  function panelText(el){try{const clone=el.cloneNode(true);clone.querySelectorAll('.v0769DamageArt,.v0769DamageInfo').forEach(x=>x.remove());return flat(clone.textContent||'')}catch(_e){return flat(el?.textContent||'')}}
  function damagePanel(){
    try{
      const marked=[...document.querySelectorAll('.v0754DamageCheckPanel')].filter(visible);if(marked.length)return marked[0];
      const walker=document.createTreeWalker(document.body||document.documentElement,NodeFilter.SHOW_TEXT);
      let n;while((n=walker.nextNode())){if(!/DAMAGE\s*CHECK/i.test(String(n.nodeValue||'')))continue;let el=n.parentElement;for(let i=0;i<6&&el;i++,el=el.parentElement){if(!visible(el))continue;const t=flat(el.textContent||'');if(/DAMAGE\s*CHECK/i.test(t)&&t.length<900){el.classList.add('v0754DamageCheckPanel');return el}}}
    }catch(_e){}return null;
  }
  function resolveCard(txt){
    const nk=norm(txt);let best=null,score=-1;
    for(const c of allDefs()){const k=norm(c?.name||'');if(k.length<2||!nk.includes(k))continue;const sc=k.length*100+(c?.no?1000:0);if(sc>score){score=sc;best=c}}
    if(best)return best;
    updateGraves();
    for(let i=recentGrave.length-1;i>=0;i--){const c=recentGrave[i].c;if(c&&Date.now()-recentGrave[i].at<2200)return c}
    return null;
  }
  function art(c){
    try{if(typeof imageTag==='function'){const h=imageTag(c,'v0769DamageImage');if(h)return h}}catch(_e){}
    try{if(typeof cardHTML==='function'){const obj=Object.assign({uid:-769001,rest:false,damage:0,tempPower:0,tempStrike:0},c||{});const t=document.createElement('div');t.innerHTML=cardHTML(obj,{select:false})||'';const card=t.querySelector('.card');if(card){card.querySelectorAll('button,.infoBtn').forEach(x=>x.remove());card.removeAttribute('onclick');card.removeAttribute('onpointerup');card.removeAttribute('onpointerdown');return card.outerHTML}}}catch(_e){}
    return '<div class="v0769DamageArtFallback">'+esc(c?.name||'公開カード')+'</div>';
  }
  function enhanceDamage(){
    updateGraves();const p=damagePanel();if(!p)return;
    const txt=panelText(p);if(!/DAMAGE\s*CHECK/i.test(txt))return;
    const c=resolveCard(txt);if(!c)return;
    const m=txt.match(/(\d+)\s*\/\s*(\d+)\s*枚目/);const lucky=/LUCKY|ラッキー/i.test(txt)||!!c?.lucky;
    const sig=String(c?.uid??c?.no??c?.name)+'|'+(m?m[1]+'/'+m[2]:'1');const now=Date.now();
    if(sig===lastDamageSig&&now-lastDamageAt<900&&p.classList.contains('v0769DamageEnhanced'))return;
    lastDamageSig=sig;lastDamageAt=now;p.classList.add('v0769DamageEnhanced');p.dataset.v0769Sig=sig;
    p.querySelectorAll(':scope > .v0769DamageArt,:scope > .v0769DamageInfo').forEach(x=>x.remove());
    const a=document.createElement('div');a.className='v0769DamageArt';a.innerHTML=art(c);p.prepend(a);
    const info=document.createElement('div');info.className='v0769DamageInfo';info.innerHTML='<div class="v0769DamageLabel">DAMAGE CHECK</div><div class="v0769DamageName">'+esc(c?.name||c?.no||'公開カード')+'</div>'+(lucky?'<div class="v0769DamageLucky">★ LUCKY!</div>':'')+(m?'<div class="v0769DamageCount">'+esc(m[1])+' / '+esc(m[2])+'枚目</div>':'');p.appendChild(info);
  }
  function cardEl(uid){
    if(uid==null)return null;const xs=[...document.querySelectorAll('.card[data-uid="'+String(uid)+'"]')].filter(visible);return xs[0]||null;
  }
  function center(el){if(!el)return null;const r=el.getBoundingClientRect();return {x:r.left+r.width/2,y:r.top+r.height/2,r}}
  function playerTarget(attOwner){
    try{const enemy=attOwner===G?.p?document.querySelector('.v04Play .side.enemy'):document.querySelector('.v04Play .side.player');if(!enemy)return null;return enemy.querySelector('.stack')||enemy}catch(_e){return null}
  }
  function targetElement(attOwner,target){if(!target)return null;if(target.type==='player')return playerTarget(attOwner);return cardEl(target.uid)}
  function speedName(){return flat(document.getElementById('v0756CpuSpeedBtn')?.textContent||'')}
  function arrowDuration(){const t=speedName();if(t.includes('即時'))return 520;if(t.includes('速い'))return 950;if(t.includes('普通'))return 1550;return 2200}
  function attackLayer(){let x=document.getElementById('v0769AttackLayer');if(!x){x=document.createElement('div');x.id='v0769AttackLayer';document.documentElement.appendChild(x)}return x}
  function ring(layer,pt,cls){const d=document.createElement('div');d.className='v0769AttackRing '+(cls||'');d.style.left=(pt.r.left-5)+'px';d.style.top=(pt.r.top-5)+'px';d.style.width=(pt.r.width+10)+'px';d.style.height=(pt.r.height+10)+'px';layer.appendChild(d)}
  function showAttack(attOwner,atk,target){
    try{
      const ae=cardEl(atk?.uid),te=targetElement(attOwner,target),a=center(ae),b=center(te);if(!a||!b)return;
      const sig=String(atk?.uid)+'|'+String(target?.type)+'|'+String(target?.uid??'player')+'|'+String(G?.turnSeq||'');const now=Date.now();if(sig===lastAttackSig&&now-lastAttackAt<900)return;lastAttackSig=sig;lastAttackAt=now;
      const layer=attackLayer();layer.innerHTML='';const ns='http://www.w3.org/2000/svg',svg=document.createElementNS(ns,'svg'),defs=document.createElementNS(ns,'defs'),marker=document.createElementNS(ns,'marker');
      marker.setAttribute('id','v0769ArrowHead');marker.setAttribute('markerWidth','12');marker.setAttribute('markerHeight','12');marker.setAttribute('refX','10');marker.setAttribute('refY','4');marker.setAttribute('orient','auto');marker.setAttribute('markerUnits','strokeWidth');const ph=document.createElementNS(ns,'path');ph.setAttribute('d','M0,0 L0,8 L11,4 z');ph.setAttribute('fill','#ffd84f');marker.appendChild(ph);defs.appendChild(marker);svg.appendChild(defs);
      const dx=b.x-a.x,dy=b.y-a.y,curve=Math.min(90,Math.max(35,Math.abs(dx)*.12+Math.abs(dy)*.08));const path=document.createElementNS(ns,'path');path.setAttribute('d','M '+a.x+' '+a.y+' Q '+((a.x+b.x)/2)+' '+((a.y+b.y)/2-curve)+' '+b.x+' '+b.y);path.setAttribute('class','v0769AttackPath');path.setAttribute('marker-end','url(#v0769ArrowHead)');svg.appendChild(path);layer.appendChild(svg);ring(layer,a,'source');ring(layer,b,'target');
      const badge=document.createElement('div');badge.className='v0769AttackBadge';let tn='攻撃対象';try{if(target?.type==='player')tn=(typeof other==='function'?other(attOwner)?.name:'PLAYER')||'PLAYER';else{const def=typeof other==='function'?other(attOwner):null;const arr=[...(def?.pals||[]),...(def?.supports||[]),...(def?.buildings||[]),...(def?.gears||[])];tn=arr.find(x=>x.uid===target.uid)?.name||tn}}catch(_e){}badge.innerHTML='<b>攻撃</b>　'+esc(atk?.name||'CPU')+' → '+esc(tn);layer.appendChild(badge);
      setTimeout(()=>{if(layer&&Date.now()-lastAttackAt>=arrowDuration()-80)layer.innerHTML=''},arrowDuration());
    }catch(_e){}
  }
  try{if(typeof declareBattle==='function'&&!declareBattle.__v0769Wrapped){const base=declareBattle;const w=function(attOwner,attUid,target,after){try{const atk=attOwner?.pals?.find(x=>x.uid===attUid);showAttack(attOwner,atk,target)}catch(_e){}return base.apply(this,arguments)};w.__v0769Wrapped=true;declareBattle=w}}catch(_e){}
  try{if(typeof resolveBattle==='function'&&!resolveBattle.__v0769ArrowWrapped){const base=resolveBattle;const w=function(attOwner,atk,target){try{showAttack(attOwner,atk,target)}catch(_e){}return base.apply(this,arguments)};w.__v0769ArrowWrapped=true;resolveBattle=w}}catch(_e){}
  function sync(){scheduled=false;try{enhanceDamage()}catch(_e){}try{if(typeof pendingBlock!=='undefined'&&pendingBlock?.atk&&pendingBlock?.target)showAttack(pendingBlock.attOwner||G?.a,pendingBlock.atk,pendingBlock.target)}catch(_e){}try{const t=document.querySelector('.v04Title');if(t&&!G?.cpuVsCpu)t.textContent='v0.7.70';document.querySelectorAll('.badge.official').forEach(b=>{if(/^v?0\.7\.\d+/.test(flat(b.textContent)))b.textContent='v0.7.70'})}catch(_e){}}
  function schedule(){if(scheduled)return;scheduled=true;requestAnimationFrame(sync)}
  try{new MutationObserver(schedule).observe(document.documentElement,{childList:true,subtree:true,characterData:true,attributes:true,attributeFilter:['class','style','hidden']})}catch(_e){}
  document.addEventListener('pointerup',schedule,{passive:true});addEventListener('pageshow',()=>{graveSeen=new Set();recentGrave=[];lastDamageSig='';lastAttackSig='';setTimeout(schedule,80)},{passive:true});setInterval(schedule,100);
  schedule();setTimeout(schedule,250);setTimeout(schedule,900);
  console.info('Palworld OCG v0.7.70 DAMAGE CHECK inline image + attack arrow + slow CPU pacing applied');
})();
</script>


<style id="v0770BattleReadabilityStyle">
/* v0.7.70 — final-source Damage Check reveal + guaranteed attack targeting cue. */
#v0770DamageOverlay{position:fixed!important;inset:0!important;z-index:2147483600!important;display:flex!important;align-items:center!important;justify-content:center!important;padding:10px!important;pointer-events:none!important;background:rgba(0,0,0,.24)!important}
#v0770DamageOverlay[hidden]{display:none!important}
.v0770DamageBox{width:min(430px,78vw)!important;max-width:430px!important;display:grid!important;grid-template-columns:138px minmax(0,1fr)!important;gap:12px!important;align-items:center!important;background:linear-gradient(180deg,#111a18fc,#07100efc)!important;border:2px solid #efd45c!important;border-radius:18px!important;padding:12px 14px!important;box-shadow:0 20px 52px #000c,0 0 30px #efd45c22!important;color:#fff!important}
.v0770DamageCardStage{display:flex!important;align-items:center!important;justify-content:center!important;min-height:176px!important}
.v0770DamageCardStage .card{width:128px!important;min-width:128px!important;max-width:128px!important;height:171px!important;min-height:171px!important;max-height:171px!important;margin:0!important;transform:none!important;pointer-events:none!important;box-shadow:0 10px 26px #000b!important}
.v0770DamageCardStage .card button,.v0770DamageCardStage .card .infoBtn{display:none!important}
.v0770DamageCardStage img{display:block!important;width:128px!important;max-width:128px!important;max-height:176px!important;height:auto!important;object-fit:contain!important;border-radius:8px!important;box-shadow:0 10px 26px #000b!important}
.v0770DamageFallback{width:128px;height:171px;border:1px solid #d6bd59;border-radius:8px;background:#16211e;display:flex;align-items:center;justify-content:center;text-align:center;padding:8px;font-size:10px;font-weight:900;color:#fff}
.v0770DamageHead{font-size:15px!important;font-weight:950!important;letter-spacing:.08em!important;color:#f7e27a!important}
.v0770DamageSide{margin-top:4px!important;font-size:9px!important;color:#cbd6d1!important}
.v0770DamageName{margin-top:7px!important;font-size:14px!important;font-weight:950!important;line-height:1.25!important;color:#fff!important}
.v0770DamageLucky{margin-top:8px!important;font-size:15px!important;font-weight:950!important;color:#ffdf58!important;text-shadow:0 0 14px #ffdf5866!important}
.v0770DamageCount{margin-top:5px!important;font-size:9px!important;color:#bdc7c2!important}
#v0770AttackLayer{position:fixed!important;inset:0!important;z-index:2147483500!important;pointer-events:none!important;overflow:hidden!important}
#v0770AttackLayer svg{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;overflow:visible!important}
.v0770AttackPath{fill:none!important;stroke:#ffe05a!important;stroke-width:9!important;stroke-linecap:round!important;filter:drop-shadow(0 0 5px #ff412d) drop-shadow(0 0 10px #000)!important;stroke-dasharray:20 11!important;animation:v0770Dash .55s linear infinite!important}
@keyframes v0770Dash{to{stroke-dashoffset:-62}}
.v0770AttackRing{position:fixed!important;border-radius:11px!important;border:4px solid #ffe05a!important;box-shadow:0 0 0 3px #e8392dcc,0 0 28px #ff493ddd!important;animation:v0770Pulse .55s ease-in-out infinite alternate!important}
.v0770AttackRing.target{border-color:#fff69a!important;box-shadow:0 0 0 4px #e52d2ddd,0 0 34px #ff493d!important}
@keyframes v0770Pulse{from{filter:brightness(.9)}to{filter:brightness(1.4)}}
.v0770AttackBadge{position:fixed!important;left:50%!important;top:8%!important;transform:translateX(-50%)!important;max-width:84vw!important;padding:7px 15px!important;border-radius:999px!important;background:#190b08f5!important;border:2px solid #ffe05a!important;color:#fff!important;font-size:11px!important;font-weight:950!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;box-shadow:0 8px 26px #000c!important}
.v0770AttackBadge b{color:#ffe05a!important}
@media(max-height:520px) and (orientation:landscape){
  .v0770DamageBox{width:min(380px,70vw)!important;grid-template-columns:104px minmax(0,1fr)!important;padding:8px 11px!important;gap:9px!important}
  .v0770DamageCardStage{min-height:128px!important}.v0770DamageCardStage .card{width:94px!important;min-width:94px!important;max-width:94px!important;height:125px!important;min-height:125px!important;max-height:125px!important}.v0770DamageCardStage img{width:94px!important;max-width:94px!important;max-height:128px!important}.v0770DamageFallback{width:94px;height:125px;font-size:8px!important}
  .v0770DamageHead{font-size:12px!important}.v0770DamageName{font-size:11px!important}.v0770DamageLucky{font-size:12px!important}.v0770AttackBadge{font-size:9px!important;padding:5px 10px!important;top:5%!important}.v0770AttackPath{stroke-width:7!important}
}
</style>
<script id="v0770BattleReadabilityScript">
(()=>{
  if(globalThis.__v0770BattleReadabilityApplied)return;
  globalThis.__v0770BattleReadabilityApplied=true;
  let damageQueue=[],damageShowing=false,damageTimer=0,attackTimer=0,holdUntil=0;
  const flat=v=>String(v==null?'':v).replace(/\s+/g,' ').trim();
  const esc=v=>String(v==null?'':v).replace(/[&<>\"]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;"}[m]));
  function stress(){try{return typeof v077Stress!=='undefined'&&!!v077Stress?.active}catch(_e){return false}}
  function cpuTurn(){try{return !!(G&&G.turn&&G[G.turn]&&G[G.turn].isAI)}catch(_e){return false}}
  function speedName(){return flat(document.getElementById('v0756CpuSpeedBtn')?.textContent||'CPU ゆっくり')}
  function attackDelay(){const t=speedName();if(t.includes('即時'))return 0;if(t.includes('速い'))return 650;if(t.includes('普通'))return 1450;return 2400}
  function attackVisibleMs(){const t=speedName();if(t.includes('即時'))return 380;if(t.includes('速い'))return 950;if(t.includes('普通'))return 1750;return 2600}
  function damageVisibleMs(){const t=speedName();if(t.includes('即時'))return 620;if(t.includes('速い'))return 1150;if(t.includes('普通'))return 1900;return 2600}
  function hideOldFx(){try{['v0758DamageReveal','v0759DamageReveal','v0760DamageReveal','v0761DamageReveal','v0765DamageReveal','v0766DamageReveal','v0767DamageReveal','v0768DamageReveal'].forEach(id=>{const x=document.getElementById(id);if(x)x.hidden=true});const x=document.getElementById('v0769AttackLayer');if(x)x.innerHTML=''}catch(_e){}}
  function damageHost(){let x=document.getElementById('v0770DamageOverlay');if(!x){x=document.createElement('div');x.id='v0770DamageOverlay';x.hidden=true;document.body.appendChild(x)}return x}
  function cardMarkup(c){
    try{if(typeof cardHTML==='function'){const obj=Object.assign({uid:c?.uid??-770001,rest:false,damage:0,tempPower:0,tempStrike:0},c||{});const t=document.createElement('div');t.innerHTML=cardHTML(obj,{select:false})||'';const card=t.querySelector('.card')||t.firstElementChild;if(card){card.querySelectorAll('button,.infoBtn,.v0754AssignOnPal,.v0754AssignOnSource').forEach(x=>x.remove());card.removeAttribute('onclick');card.removeAttribute('onpointerup');card.removeAttribute('onpointerdown');return card.outerHTML}}}catch(_e){}
    try{if(typeof imageTag==='function'){const h=imageTag(c,'v0770DamageImage');if(h)return h}}catch(_e){}
    return '<div class="v0770DamageFallback">'+esc(c?.name||c?.no||'公開カード')+'</div>';
  }
  function renderDamage(item){
    hideOldFx();const h=damageHost(),c=item.card||{};const count=item.total>1?'<div class="v0770DamageCount">'+item.index+' / '+item.total+'枚目</div>':'';
    h.innerHTML='<div class="v0770DamageBox"><div class="v0770DamageCardStage">'+cardMarkup(c)+'</div><div><div class="v0770DamageHead">DAMAGE CHECK</div><div class="v0770DamageSide">'+esc(item.defName?item.defName+' のダメージチェック':'ダメージチェック')+'</div><div class="v0770DamageName">'+esc(c.name||c.no||'公開カード')+'</div>'+(c.lucky?'<div class="v0770DamageLucky">★ LUCKY!</div>':'')+count+'</div></div>';
    h.hidden=false;const d=damageVisibleMs();holdUntil=Math.max(holdUntil,Date.now()+d+250);
  }
  function pumpDamage(){if(damageShowing||!damageQueue.length)return;damageShowing=true;const item=damageQueue.shift();renderDamage(item);clearTimeout(damageTimer);damageTimer=setTimeout(()=>{const h=document.getElementById('v0770DamageOverlay');if(h)h.hidden=true;damageShowing=false;pumpDamage()},damageVisibleMs())}
  function enqueueDamage(cards,def){if(!cards?.length)return;cards.forEach((c,i)=>damageQueue.push({card:c,defName:def?.name||'',index:i+1,total:cards.length}));if(damageQueue.length>12)damageQueue=damageQueue.slice(-12);pumpDamage()}

  /* FINAL wrapper: installed after all older diagnostics/UX patches so it cannot be overwritten by them. */
  try{
    if(typeof playerDamage==='function'&&!playerDamage.__v0770FinalDamageWrapped){
      const base=playerDamage;
      const wrapped=function(def,amount){
        const n=Math.max(0,Number(amount)||0),revealed=[];
        try{const deck=Array.isArray(def?.deck)?def.deck:[];for(let i=0;i<n&&i<deck.length;i++){const c=deck[i];if(!c)continue;revealed.push(c);if(c.lucky)break}}catch(_e){}
        const r=base.apply(this,arguments);
        try{if(n>0&&revealed.length)enqueueDamage(revealed,def);else if(n>0&&typeof palDiagRecord==='function')palDiagRecord('ERR-DMG-0110','playerDamage fired but no reveal card could be captured',{op:'v0770PlayerDamage',amount:n,deck: def?.deck?.length??null},null,true)}catch(_e){}
        return r;
      };
      wrapped.__v0770FinalDamageWrapped=true;wrapped.__v0770FinalDamageOld=base;playerDamage=wrapped;try{globalThis.playerDamage=wrapped}catch(_e){}
    }
  }catch(e){try{if(typeof palDiagRecord==='function')palDiagRecord('ERR-DMG-0111','failed to install final playerDamage reveal hook',{op:'v0770InstallDamage'},e,true)}catch(_e){}}

  function visible(el){try{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return r.width>8&&r.height>8&&s.display!=='none'&&s.visibility!=='hidden'&&Number(s.opacity||1)>0}catch(_e){return false}}
  function cardElement(uid){if(uid==null)return null;const xs=[...document.querySelectorAll('.v04Play [data-uid="'+String(uid)+'"]')].filter(visible);if(!xs.length)return null;xs.sort((a,b)=>{const ar=a.getBoundingClientRect(),br=b.getBoundingClientRect();return (br.width*br.height)-(ar.width*ar.height)});return xs[0]}
  function pointFromEl(el){if(!el)return null;const r=el.getBoundingClientRect();return {x:r.left+r.width/2,y:r.top+r.height/2,r}}
  function playerPoint(def){
    try{
      const name=flat(def?.name||'');let best=null,bestArea=Infinity;
      for(const el of document.querySelectorAll('.v04Shell div,.v04Shell section,.v04Shell aside')){if(!visible(el))continue;const t=flat(el.textContent||'');if(!/LIFE/i.test(t))continue;if(name&& !t.includes(name))continue;const r=el.getBoundingClientRect(),area=r.width*r.height;if(r.width>55&&r.width<340&&r.height>25&&r.height<190&&area<bestArea){best=el;bestArea=area}}
      if(best)return pointFromEl(best);
    }catch(_e){}
    const cpu=def===G?.a;const x=innerWidth*.875,y=innerHeight*(cpu?.20:.76);return {x,y,r:{left:x-46,top:y-24,width:92,height:48}};
  }
  function targetPoint(attOwner,target){if(!target)return null;if(target.type==='player'){let def=null;try{def=typeof other==='function'?other(attOwner):(attOwner===G?.p?G?.a:G?.p)}catch(_e){}return playerPoint(def)}return pointFromEl(cardElement(target.uid))}
  function sourcePoint(atk){return pointFromEl(cardElement(atk?.uid))}
  function attackLayer(){let x=document.getElementById('v0770AttackLayer');if(!x){x=document.createElement('div');x.id='v0770AttackLayer';document.documentElement.appendChild(x)}return x}
  function ring(layer,p,cls){const d=document.createElement('div');d.className='v0770AttackRing '+(cls||'');d.style.left=(p.r.left-5)+'px';d.style.top=(p.r.top-5)+'px';d.style.width=(p.r.width+10)+'px';d.style.height=(p.r.height+10)+'px';layer.appendChild(d)}
  function targetName(attOwner,target){try{const def=typeof other==='function'?other(attOwner):(attOwner===G?.p?G?.a:G?.p);if(target?.type==='player')return def?.name||'プレイヤー';const arr=[...(def?.pals||[]),...(def?.supports||[]),...(def?.buildings||[]),...(def?.gears||[])];return arr.find(x=>x.uid===target?.uid)?.name||'攻撃対象'}catch(_e){return '攻撃対象'}}
  function showAttack(attOwner,atk,target){
    try{
      hideOldFx();const a=sourcePoint(atk),b=targetPoint(attOwner,target);if(!a||!b){if(typeof palDiagRecord==='function')palDiagRecord('ERR-ATK-0101','attack target arrow coordinates could not be resolved',{op:'v0770AttackArrow',atk:atk?.name||'',targetType:target?.type||'',targetUid:target?.uid??null},null,true);return}
      const layer=attackLayer();layer.innerHTML='';const ns='http://www.w3.org/2000/svg',svg=document.createElementNS(ns,'svg'),defs=document.createElementNS(ns,'defs'),marker=document.createElementNS(ns,'marker');marker.setAttribute('id','v0770ArrowHead');marker.setAttribute('markerWidth','13');marker.setAttribute('markerHeight','13');marker.setAttribute('refX','10');marker.setAttribute('refY','4');marker.setAttribute('orient','auto');marker.setAttribute('markerUnits','strokeWidth');const ph=document.createElementNS(ns,'path');ph.setAttribute('d','M0,0 L0,8 L11,4 z');ph.setAttribute('fill','#ffe05a');marker.appendChild(ph);defs.appendChild(marker);svg.appendChild(defs);
      const curve=Math.min(110,Math.max(38,Math.abs(b.x-a.x)*.13+Math.abs(b.y-a.y)*.08)),path=document.createElementNS(ns,'path');path.setAttribute('d','M '+a.x+' '+a.y+' Q '+((a.x+b.x)/2)+' '+((a.y+b.y)/2-curve)+' '+b.x+' '+b.y);path.setAttribute('class','v0770AttackPath');path.setAttribute('marker-end','url(#v0770ArrowHead)');svg.appendChild(path);layer.appendChild(svg);ring(layer,a,'source');ring(layer,b,'target');const badge=document.createElement('div');badge.className='v0770AttackBadge';badge.innerHTML='<b>攻撃</b>　'+esc(atk?.name||'CPU')+' → '+esc(targetName(attOwner,target));layer.appendChild(badge);
      clearTimeout(attackTimer);const d=attackVisibleMs();holdUntil=Math.max(holdUntil,Date.now()+d);attackTimer=setTimeout(()=>{if(layer)layer.innerHTML=''},d);
    }catch(_e){}
  }
  try{if(typeof declareBattle==='function'&&!declareBattle.__v0770ArrowWrapped){const base=declareBattle;const wrapped=function(attOwner,attUid,target,after){try{const atk=attOwner?.pals?.find(x=>x.uid===attUid);showAttack(attOwner,atk,target)}catch(_e){}return base.apply(this,arguments)};wrapped.__v0770ArrowWrapped=true;wrapped.__v0770ArrowOld=base;declareBattle=wrapped;try{globalThis.declareBattle=wrapped}catch(_e){}}}catch(_e){}

  /* Hold CPU battle resolution after declaration so the arrow can actually be read. */
  try{if(typeof quickWindowThenResolve==='function'&&!quickWindowThenResolve.__v0770SlowWrapped){const base=quickWindowThenResolve;const wrapped=function(attOwner,atk,target,after){let cpu=false;try{cpu=!!attOwner?.isAI||attOwner===G?.a}catch(_e){}const d=(!stress()&&cpu)?attackDelay():0;if(d>0){try{showAttack(attOwner,atk,target)}catch(_e){}holdUntil=Math.max(holdUntil,Date.now()+d);setTimeout(()=>base.apply(this,arguments),d);return}return base.apply(this,arguments)};wrapped.__v0770SlowWrapped=true;wrapped.__v0770SlowOld=base;quickWindowThenResolve=wrapped;try{globalThis.quickWindowThenResolve=wrapped}catch(_e){}}}catch(_e){}

  /* Do not let the next scheduled CPU action cover the Damage Check reveal. */
  try{if(typeof v070Schedule==='function'&&!v070Schedule.__v0770HoldWrapped){const base=v070Schedule;const wrapped=function(fn,ms){let d=Math.max(0,Number(ms)||0);if(!stress()&&cpuTurn()&&holdUntil>Date.now())d=Math.max(d,holdUntil-Date.now()+180);return base.call(this,fn,d)};wrapped.__v0770HoldWrapped=true;wrapped.__v0770HoldOld=base;v070Schedule=wrapped;try{globalThis.v070Schedule=wrapped}catch(_e){}}}catch(_e){}

  function version(){try{const t=document.querySelector('.v04Title');if(t&&!G?.cpuVsCpu)t.textContent='v0.7.70';document.querySelectorAll('.badge.official').forEach(b=>{if(/^v?0\.7\.\d+/.test(flat(b.textContent)))b.textContent='v0.7.70'})}catch(_e){}}
  addEventListener('pageshow',()=>{damageQueue=[];damageShowing=false;holdUntil=0;clearTimeout(damageTimer);clearTimeout(attackTimer);const d=document.getElementById('v0770DamageOverlay');if(d)d.hidden=true;const a=document.getElementById('v0770AttackLayer');if(a)a.innerHTML='';setTimeout(version,60)},{passive:true});
  setInterval(version,500);version();
  console.info('Palworld OCG v0.7.70 final Damage Check source hook + guaranteed attack arrow + CPU readability hold applied');
})();
</script>
...
export default{
  async fetch(request){
    ...
    try{
      ...
      h.set("x-palworld-bridge","v0.7.70-battle-readability");
      ...
      if(u.pathname==="/manifest.webmanifest"){
        ...
        m.description="Palworld OCG v0.7.70 — DAMAGE CHECK実カード直接表示・攻撃対象矢印・CPU攻撃待機・Tier候補合法性ガード・全検査・診断・公式ルール同期";
        m.start_url="/?pwa=1&v=0770";
        m.scope="/";
      ...
      if(u.pathname==="/sw.js"){
        let sw=await r.text();
        sw+="\n// v0.7.70 final damage source hook + attack arrow + CPU readability hold + tier legal guard + diagnostics\n";
        h.delete("content-length");
        h.delete("content-encoding");
        h.delete("etag");
